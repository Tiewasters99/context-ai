...
| `_verify-stepup-seal.mjs` | Migration 094 (S1) — a sealed matter opens only to a session that has confirmed a second factor. As `authenticated` with PostgREST-shaped claims: aal1 refused on a sealed matter, on a sealed sub-matter under an open parent, and on an open-tier matter under a sealed one — including for a member who cannot see the sealed parent; aal2 reads all of them and nothing membership does not allow; a token our server minted for a connector (`cs_via`) is not gated and one with neither aal2 nor the stamp is; E2's grace by sign-in time for someone with no factor. `matter_entry` answers `stepup`/`enrol` only to members; `search_passages` / `search_documents` / `search_conversations` hand their definer core no sealed id at aal1 (the cores are scope-echo stubs, so the witness is the scope); the passages policy is untouched. The fifteen new Record kinds land, an unknown one is refused, `EVENT_KINDS` / `ACCOUNT_EVENT_KINDS` equal the database's lists, `events (actor_ref, ts desc)` exists; the devices functions are service-role only. Negative control first: without 094 an aal1 session reads the seal. Last, `api/account-sessions.mjs` and `api/account-factor-event.mjs` driven with `fetch` stubbed. | PGlite + pgvector, real roles and real RLS, 001→073 then 094 twice; the endpoints get an injected `fetch` and ledger client. |
| `_verify-storage-path.mjs` | Migration 097 — a document's stored file lives under its own matter. The CHECK `documents_storage_path_in_matter` refuses a cross-matter path on INSERT and UPDATE and a move of the matter that leaves the path behind; accepts a same-matter path, a null path and a move that rewrites both columns in one UPDATE. Then the pointer-row attack (an open-matter row naming a sealed object) against `/api/cloud-export` (OneDrive, Dropbox), `/api/drive-export`, `/api/ext/push-to-drive`, `/api/gmail-send`, `/api/ingest` and `get_media`: each refuses with zero storage reads and nothing sent outside, and each gets past the check with the row filed correctly (control). Last, `lib/storage-path.mjs` itself and a scan that the routes, get_media and the worker's ingest ask it. Round 2: the CHECK and every site also refuse traversal rows that START with the right matter (`../`, `..\`, `%2e%2e`, `.`, `//`, four segments, an upper-case uuid); `production_items` paths are held to `<matter>/<production>/…`; mcp-core's move/copy/assemble/edit_pdf refuse them with a service-role client; the discovery worker checks job and item paths before any download and refuses a queued `intake_folder`. | PGlite for the constraint; the handlers get `fetch` answering as Supabase and the outside services. |
| `_verify-disconnect-all.mjs` | Migration 095 (S2) — "Disconnect everything" and "Disconnect all assistants from this matter". The preview names exactly what the press touches and changes nothing; matter scope takes only the presser's own connections that reach the matter (full-access ones outright; agents listed on it, on a parent, or all-matters — not one scoped elsewhere) and pauses it; a member who is not an owner/admin is refused by the wrapper and the definer core. Account scope flips every grant, token and charter of the presser, pauses every root matter of the serverspaces they run (an already-paused one keeps its note; a sub-matter inherits; a matter admin who does not run the firm pauses nothing of it) and writes `connector.revoked {by:'kill'}`, `account.locked` and per-root `ai.paused` on the right chains. Every revoked credential is then refused on the next request by the existing checks (`api/mcp.mjs` authenticate, `lib/connector-token-auth.mjs`, `lib/oauth-grants.mjs`, the refresh grant) while another customer's still work; a failure at the last step (the `account.locked` row) rolls everything back; `account.unlocked` is written once, on the first resume or reconnection after a lock. Last, `api/account-lockdown.mjs`: no bearer → 401 and no request; a database refusal → 502 and no sign-out; success → `logout?scope=others` with the presser's own token. | PGlite, real roles and real RLS, 001→088, 094, then 095 twice; a stub PostgREST + Auth over the same database drives the real handlers. |
| `_verify-brief-md-roundtrip.mjs` | Migration 100 and the Brief Desk, slice D1 (`docs/specs/BRIEF-DESK-2026-09-26.md`). **The dialect** (`lib/brief-md.mjs`): a Webster-style master with footnotes, flags, blockquotes, a hard break, passthrough lines and a signature block round-trips **byte-identical**; a hurried spelling of it (CRLF, word labels, no blank lines, `**[OPP …]**`) gives the same notes; an undefined, doubly defined or unused footnote label loses nothing and the output never has one (the house-style build stops on either); text that looks like markup comes back as the same words; highlight and cite marks never reach the .md; bold-italic the build cannot express is written without the italic **and reported**; `toPlainText` (what the cite-check reads) has no markup. **The schema**: every parsed document loads into the real TipTap schema, passes `check()` and comes back identical; no list, code, rule, strike or link node exists to be pasted in. **Word in**: a generated .docx (bold headings, two notes, numbering, a table, a tracked change, a TOC field) imports with each note where its reference was and a loss list that names exactly those losses and nothing else. **Word out**: one `w:footnoteReference` per note, each backed in `word/footnotes.xml`; the DRAFT line; flags red. **Migration 100**: after 094 and on a pre-094 database the constraint equals `EVENT_KINDS` (the UNION rule), twice; member / viewer / stranger on the three tables; a snapshot is frozen (no update or delete, even by the owner) and its sha256 is the database's, not the client's; the optimistic lock; a note is written, edited and deleted only by its author; the three Record kinds land through `ledger_append`. | Node + PGlite, real roles and real RLS, 001→073, 094's vocabulary block from the real file, then 100 twice. Needs `--import ./scripts/_node-src-loader.mjs` (schema, import and export are TypeScript). The Word fixture is generated at run time; no binary in the repo. |
...
| `_verify-entitlements.mjs` | **Hidden also means locked.** Part A is static and needs nothing installed: every surface in `lib/surfaces.mjs` — the one table the browser, the API handlers and migration 083 all read — must declare how the SERVER holds it shut (an endpoint gate, an RLS gate, or an explicit `public-by-design` / `front-end-only` / `core-open` note), and the declaration is then CHECKED: a named endpoint must really call `requireEntitlement()` with that surface's id, a named table must really be fenced in 083, 083's own core list must match the table, `lib/surfaces.d.mts` must name the same surfaces, and `src/lib/plan.ts` must not have grown a second copy. **A new frozen or beta surface that declares nothing turns this red** — the point being that the next hidden room cannot ship unlocked in silence. Part B runs 001 → 005 → 008 → 016 → 022 → 034 → 037 → 039 → 041 → 052 → 062 → 083 in PGlite with three accounts (free, pro, workshop), opening with the hole as a **negative control** (before 083 a free account starts a Moot Bench session), then: `plan_can_open()` agrees with `canOpenSurfaceId()` on every surface × every tier (so the SQL copy of the list cannot drift), free and pro are refused the INSERT that starts work on all five fenced tables while workshop and the service role are not, the row made before 083 is still readable, editable and deletable by its owner, the Student Hub invitation cap counts and refuses in plain words and is callable by `service_role` alone, `profiles` is fingerprinted before and after and must be identical (062 and 067 own that table), and 083 is idempotent and applies to a database missing one of the tables. | PGlite, real roles, real RLS. Shares one CI step with the harness below. **Its step sits with the other PGlite steps**, not at the end of `ci.yml` — the `npm i --no-save jsdom` steps prune PGlite. |
| `_test-entitlement-handlers.mjs` | The handler half. Drives the real `api/mediation.mjs`, `api/meeting-chat.mjs`, `api/meeting-flag.mjs`, `api/student-hub-ocr.mjs` and `api/student-hub-invite.mjs` with `globalThis.fetch` replaced by a **witness**, so each case asserts what was contacted as well as what came back: a free account gets 403 and the sentence "This room is not part of your plan.", an unreadable plan is retried once and then 503 (never a 403 — a workshop account must not be told it has been demoted), a brand-new account with no `profiles` row reads as `free` without a retry, a core surface is answered with **no database round trip at all**, and nothing a caller says about its own plan is consulted. For Mediation: each of the four `runMediator()` calls is metered BEFORE the provider is contacted (asserted on the order the witness saw), a spent wallet or a full rate window returns 402/429 with **zero** model calls, and a case already on foot is not stopped by the plan gate. For the Student Hub: the invitation is charged before the mail goes out, a capped seat sends no mail at all, and the cap fails open while it is undeployed and closed once it exists. | `node --test`. Offline by construction — no network, no database, no `.env`. |
...
| `_test-transcript-parse.mjs` | Transcript chunking (blank numbered lines, Veritext text layers, letter-spaced witness names, condensed sheets) **and** the reporter's printed page: the exhibit-wrapped −1 offset, front matter, volume 2 starting at 214, TXT printouts, OCR noise, and the ambiguous document that claims nothing. Part 3 holds the page break: the number the reporter printed is not a line, so a page cannot be cited "35:35-7" and no line-number token is left inside a passage — including on a page with no numbers in the text layer. | Synthetic fixtures built in the file; no network and no client transcript. |
| `_test-eml-headers.mjs` | An email's headers and its body go through the same charset: a raw UTF-8 em dash in an 8-bit `Subject:` (the "Approval â validation approach change" mojibake), RFC 2047 encoded words in B and Q decoded once from the charset they name, adjacent encoded words folded, windows-1252, multipart (the first text part's charset reads the top headers), attachment filenames, and ASCII headers byte-for-byte unchanged. | Messages built as bytes inside the file; no network and no real mail. |
| `_verify-cite-printed-page.mjs` | The display half of the same bug: a citation SHOWS the reporter's printed page, and where the detector declined it cites the PDF page **and says so in the citation itself**. Opens with a snapshot of `origin/main`'s own citations — no passage carries the new keys until its document is re-indexed, so a key-less passage must still cite character for character, and a red line there blocks the change whatever else passes. Then the three states through `lib/cite-page.mjs`, the invariant that a printed page never appears beside a caveat, condensed sheets unchanged, the corrected `grep` citation (`p. 1-undefined:1` → the page alone on prose, `page:line` only where the reporter's line numbers were read), and the extra metadata query held to **one** query, scoped to the ids being returned, and never fatal — failing it or throwing inside it leaves the search answering with today's citations. | A stub client that records every query and can be made to fail the metadata read; no PGlite, no network, no `.env`. |
...
| `_test-text-anchor.mjs` | Marks on documents without pages (Word, text, markdown, screenplay): `src/lib/text-anchor.ts` turns offsets into a `{start, end, exact, prefix, suffix}` anchor (edge whitespace trimmed, context clipped at 32), gives back the same offsets while the text is unchanged, finds the right one of several identical quotes by prefix/suffix after the text has shifted (context beats nearness to the stale offset), and returns null when the words are gone, which the reader lists as unplaced. The DOM half (selection to offsets, offsets to a Range) is not covered here. | `node --test` with `node --import ./scripts/_node-src-loader.mjs`; pure computation, no DOM, no network, no `.env`. |
...
| `_test-grep-list-routes.mjs` | grep's routes on a stand-in database: a literal with words goes through the full-text index (confirmed by ILIKE); a regex or a word fragment scans the documents in batches and reports completeness; list_matter_contents pages through 2,500 documents with the true total and next_offset. | Pure; stand-in client; reads no `.env`. |
| `_test-unreadable-text.mjs` | A text layer of glyph codes (a browser-printed PDF with Type 3 fonts) is unreadable, while prose, Spanish, figures and dockets are not; an unreadable page is OCR'd even with no image on it; a web page saved without an extension sniffs as HTML. | Pure computation; pdf-lib builds the one fixture PDF; reads no `.env`. |
| `_test-bluebook.mjs` | Bluebook cites for Westlaw opinions: reporter spelling (R6.1), pin ranges (R3.2(a)), T1/T7 courts, T6 case names from Westlaw file names, a federal appeal, a Supreme Court download (U.S. confirmed by first page and year), New York with the regional reporter, a WL-only decision, and no cite for an unknown court. | Pure computation over fictional fixtures; reads no `.env`. |
| `_test-westlaw-pages.mjs` | Westlaw star pages: a reporter case paged from its header cite and `*N` markers, decoy cites (`at *3`, `WL …, *4`, `id. at *9`) skipped, a parallel `**` reporter, footnotes left unpaged, a restarting pagination declined, and `citePage` reading the result as the printed page. | Pure computation over fictional fixtures; reads no `.env`. |
...
| `_verify-connections-columns.mjs` | Migration 080 — the browser cannot read a connection's stored token, even encrypted. Creates `anon` / `authenticated` / `service_role` (BYPASSRLS) for real, applies Supabase's own blanket `grant all` so there is something to undo, runs 026 → 029 → 075 → 080, and asserts from inside `SET ROLE` with `request.jwt.claims` set. Opens with the leak as a **negative control** (before 080 the ciphertext is served, to `select=*` as well), then: `encrypted_refresh_token` is refused with `42501`, **`select *` is refused too — loudly, which is why every browser read of this table must name its columns**, the safe columns are served and RLS still cuts them to the caller's own rows, another user's rows stay invisible, and the token cannot be used as an oracle through `WHERE` or `ORDER BY` either. Then: `service_role` still reads and writes every row and every column; the Connections page's Disconnect (DELETE by id) still works and still cannot reach another user's row while INSERT and UPDATE are refused; 080 run twice leaves the column ACLs byte-identical; and it converges from three drift states — 029 never applied, no `connections` table at all, and someone having hand-granted the token column. Ends with two static scans: **every file under `src/`** — none may select `*` from `connections` or name a credential column, and `CONNECTIONS_SAFE_SELECT` must be a subset of the migration's own safe list (read out of the `.sql` file, so the harness cannot drift from the thing it verifies) — and **every file under `api/` and `lib/`**, where all 17 call sites are traced to the client they run on: each one naming the token column must be on a client built from `SUPABASE_SERVICE_ROLE_KEY`, and the one user-scoped read (`api/cloud-export.mjs`, the account label) must name only granted columns. | PGlite, real roles, real RLS. No `.env`, no network. **Its step sits with the other PGlite steps, not at the end of `ci.yml`**: the two `npm i --no-save jsdom` steps near the bottom prune PGlite out of the tree (npm removes what is not in `package.json`), so a PGlite harness appended after them exits 2. That is also why `_verify-cloud-drives.mjs` prints SKIP for its migration-075 section in CI today. |
...
| `_test-first-run.mjs` | The first five minutes of a brand-new account: who the Dashboard's first-run docket is shown to (never `workshop`, never while the plan or the serverspaces list is still being read, never again once dismissed — and still shown, with ticks, to an account part-way through); that every tick derives from a fact and an unreadable fact is `null` rather than a tick; that a second click on "create your workspace" writes nothing and returns the one that exists; that the sealed-from-the-start option reaches `ai_tier: 'B'` in the product's own New Matter dialog; and that the copy claims only what the product keeps — the 500 MB cap and the accepted-types list compared against `lib/ingest-formats.mjs`, the ledger kinds against the Record's `describe.ts`, `?tab=Record` against `MatterspaceView`'s own tab list, and the sealed sentence forbidden from naming a model. | The rules are pure functions in `src/components/firstrun/first-run.ts` (no React, no Supabase); the create path takes its client as an argument and is driven against `scripts/_fake-supabase.mjs`; the React wiring is asserted as source text. `node --import ./scripts/_node-src-loader.mjs` for the `@/lib/plan` import. |
