...
| `_verify-disconnect-one-way.mjs` | Migration 099 (S2b) — a disconnection is one-way, and the Record says who reconnected. After the press no UPDATE brings a connector token back (revoked_at, expires_at, kind, matter_scope, scope_all, user_id, token_hash — from a stolen sign-in or the owner's own); a pre-065 (gid-less) client is refused on refresh, cspa_ and bare-JWT paths and is never adopted; a sign-in issued before the lock (stolen, the unrefreshed tab, the lock's own second, no iat, a connector's minted token) cannot paste a token, approve a grant, enable a charter, widen an agent or resume AI, and writes no `account.unlocked`; one issued after it can, and the single `account.unlocked` row carries its `session_id` and `iat` (through the consent screen too), while the pre-lock sign-in stays refused. Also: the account press needs aal2 when there is a factor (endpoint → `step_up_required`, nothing revoked; the RPC refuses too); a serverspace admin's own press pauses nothing of the firm's; `/api/ext/matters`, `/documents` and `/push-to-drive` refuse a paused matter (fail closed); the grant cache is not trusted while the owner is locked and an unreadable grant fails closed only then. | PGlite, real roles and real RLS, 001→088, 094, 095, then 099 twice; a stub PostgREST + Auth over the same database drives the real handlers. |
