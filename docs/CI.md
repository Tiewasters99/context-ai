...
| `_verify-sealed-download.mjs` | Migration 096 (S4a) — every copy that leaves a sealed matter is in its Record. Negative control first: without 096 an aal2 member reads a sealed object directly. Then, as `authenticated`: a sealed object (own tier or inherited) is refused in `vault-documents` and `discovery-files`, open ones still read, the service role unaffected, a stranger and anon read nothing; a NEW upload into a sealed matter still works (`INSERT … RETURNING`, the `created_at = now()` clause) while overwrite, rename and a `created_at` rewrite of an existing sealed object reach nothing; 049's UPDATE policy unchanged and none added; 096 refuses to run without 094. `api/document-url.mjs` and `get_media` driven with `fetch` answering as Supabase from the database (storage's sign = a SELECT as the caller): aal2 member mints + `file.opened` (read) / + `file.exported` (download); aal1 gets `step_up_required` with nothing minted or written; open matter read writes nothing, download writes `file.exported {destination:'download'}`; non-member, no bearer, connector-stamped token, a row pointing at another matter's object and a `..` path all refused before any mint; a Record that cannot be written on a sealed matter hands out nothing. Last, a scan: no file in `src/` signs or downloads from either bucket except `src/lib/vault-object.ts`. | PGlite, real roles and real RLS, 001→094 then 096 twice; the endpoint gets an injected `fetch`. |
