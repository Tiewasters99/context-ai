...
| `_verify-disconnect-one-way.mjs` | Migration 099 (S2b) — a disconnection is one-way, and the Record says who reconnected. After the press no UPDATE brings a connector token back (revoked_at, expires_at, kind, matter_scope, scope_all, user_id, token_hash); a pre-065 (gid-less) client is refused on refresh, cspa_ and bare-JWT paths and is never adopted; the lock is judged by SESSION: a session from before the press other than the presser's — signed out, refreshed, surviving an RPC-only press — a connector's minted token, or a token with no session cannot paste a token, approve a grant, enable a charter, widen an agent or resume AI, and writes no `account.unlocked`; the presser's own session (no refresh) and a sign-in made after the press can, and the single `account.unlocked` row carries its `session_id` and `iat` (through the consent screen too); a service-role grant with no sign-in stays refused after an unlock. The press is never gated on a factor; only the sign-out of others is (a factor newer than the presser's session does not count); a serverspace admin's own press pauses nothing of the firm's; `/api/ext/matters`, `/documents` and `/push-to-drive` refuse a paused matter (fail closed); the grant cache is not trusted while the owner is locked. | PGlite, real roles and real RLS, 001→088, 094, 095, then 099 twice; a stub PostgREST + Auth (with auth.sessions) over the same database drives the real handlers. |
