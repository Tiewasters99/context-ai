...
1. **Privileged container as a sealed unit.** Folders and sub-matters are the same container and `ai_tier` inherits downward, so a sealed sub-matter inside an open matter already exists. The connector half of the isolation is now CLOSED (2026-09-20): `search` and `grep` scoped to a parent expand it to its descendants (`lib/mcp-core.mjs`, matterspace_descendants), and that expanded list is filtered by `opts.excludeMatterIds`, so an external connector can no longer reach a sealed child through its open parent — proved offline by `scripts/_verify-sealed-descendants.mjs`. In-app sessions are deliberately unchanged: a member searching a parent still sees its sealed children, because the seal governs what leaves to an outside model. What remains of this item is the SESSION grain — excluding a descendant whose effective tier exceeds the *session's* tier from in-app retrieval, outline and listing; outbound-tool absence for sealed contexts already holds at matter grain and carries over.
