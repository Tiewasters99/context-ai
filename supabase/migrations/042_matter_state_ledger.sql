...
-- ============================================================================
-- 0. Re-assert the canonical activity_feed view (024 + 025).
--
-- Discovered on first apply (2026-07-30): the LIVE view had drifted to an
-- older shape (matterspace_id / text / created_at ...) that matches neither
-- migration 024/025 nor the app — useActivityFeed selects matter_id /
-- occurred_at, so the Updates tab and Dashboard feed were silently failing
-- and rendering empty. Recreating the view here both fixes that and gives
-- this migration's functions the columns they compile against. DROP is
-- required because Postgres cannot rename view columns in place; nothing
-- else depends on the view (if a dependent has appeared since, the error
-- will name it — resolve rather than CASCADE).
-- ============================================================================
