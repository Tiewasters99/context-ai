...
import { aiPausedMessage, matterPauseWithClient, pausedMatterIds } from './ai-tier-policy.mjs';
...
// ---------------------------------------------------------------------------
// The matter pause (migration 070), for the extension's endpoints
// ---------------------------------------------------------------------------
// A paused matter is refused to every connector, agent and chat, whoever
// connected it (095's header says so, and since 099 it is true here too). A
// pasted csp_ token is a connected app: a co-counsel's token must not list a
// paused matter's documents or push its bytes to Drive. Read with the SERVICE
// role — the pause, like the seal, is policy, and a paused ancestor the caller
// cannot see still pauses the child — and fails CLOSED: a pause that cannot
// be read refuses, exactly as lib/mcp-core.mjs enforceAiPause does. Before 070
// is pasted there is no pause, and nothing is refused.

/**
 * @returns {Promise<null | {status:number, body:object}>} null = not paused
 */
export async function pausedMatterRefusal(matterId, { client = null } = {}) {
  if (!matterId) return null;
  const pause = await matterPauseWithClient(client ?? adminClient(), matterId);
  if (pause?.notDeployed) return null;
  if (pause?.error) return { status: 503, body: { error: 'pause_unverifiable' } };
  if (pause?.paused) return { status: 403, body: { error: 'ai_paused', message: aiPausedMessage(pause) } };
  return null;
}

/**
 * Every paused matter id (effective, descendants included), or null when the
 * pause could not be read — the caller must then refuse or hide everything.
 */
export async function pausedMatterSet({ client = null } = {}) {
  try {
    const { ids } = await pausedMatterIds(client ?? adminClient());
    return ids;
  } catch {
    return null;
  }
