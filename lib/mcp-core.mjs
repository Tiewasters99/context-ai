...
import { ROUTES, routeForTier } from './embed-routes.mjs';
...
export async function handleSearch(supabase, args, opts = {}) {
  ...
  const unembeddable = byRoute.get('__none__')?.ids ?? [];
  const sealedNote = unembeddable.length
    ? sealedSearchNote(unembeddable.length, matterIds.length)
    : null;
  ...
  // route's batches. A route that is configured but keyless embeds nothing
  ...
  for (const { route, ids } of byRoute.values()) {
    ...
    const key = route.provider === 'openai'
      ? (opts.openaiApiKey ?? process.env[route.keyEnv])
      : process.env[route.keyEnv];
    if (!key) {
    ...
    groups.push({
      ids,
      embedding: await embedOne(key, args.q, route),
      model: route.model,
    });
  }
...
export async function embedOne(apiKey, text, route = ROUTES['openai-3-small']) {
  const res = await fetchWithTimeout(20000, `${route.provider} embeddings`)(route.url, {
    method: 'POST',
