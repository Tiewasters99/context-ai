...
import { partitionSealedMatters, sealedSearchNote } from './seal-pipes.mjs';
...
export const TOOLS = [
  ...
  },
  {
    ...
    description:
      ...
      'check_ingest_status). Name the target matter explicitly (call ' +
      'list_matters first if unsure). Provide the file as text, or as base64 ' +
      'for binary documents (PDF/DOCX/media). Returns the new document_id ' +
      'and the number of passages indexed.',
    inputSchema: {
      type: 'object',
      properties: {
        matter: {
  ...
  },
  {
    ...
    description:
      ...
      'seek, or download directly. Never tell the user media is unavailable ' +
      'from Contextspaces, and never ask them for a local file path — this ' +
      'tool is the supported route. Find the document UUID first via ' +
      'list_matter_contents (media documents carry source_filename and ' +
      'media_kind fields) or search. The URL expires after expires_in ' +
      'seconds (default 900); simply call again for a fresh link.',
    inputSchema: {
      type: 'object',
      properties: {
        document_id: {
...
export async function handleSearch(supabase, args, opts = {}) {
  ...
  const { open: openIds, sealed: sealedIds } =
    await partitionSealedMatters(supabase, matterIds);
  const sealedNote = sealedIds.length
    ? sealedSearchNote(sealedIds.length, matterIds.length)
    : null;

  let queryEmbedding = null;
  if (openIds.length) {
    if (!opts.openaiApiKey) throw new Error('openaiApiKey is required for search');
    queryEmbedding = await embedOne(opts.openaiApiKey, args.q);
  }
  const limit = args.limit ?? 5;

  // `embedding` is the parameter, not a lookup: each group of matter ids is
  // dispatched with its own, and the sealed group's is always null. There is
  // no path by which a sealed id reaches an RPC carrying a query embedding.
  const rpcParams = (ids, embedding) => ({
  ...
  // The two halves of the scope, each with the embedding it is allowed.
  const groups = [
    ...(openIds.length ? [{ ids: openIds, embedding: queryEmbedding }] : []),
    ...(sealedIds.length ? [{ ids: sealedIds, embedding: null }] : []),
  ];
  ...
  if (matter) {
    const settled = await Promise.all(
      groups.map((g) => supabase.rpc('search_passages', rpcParams(g.ids, g.embedding)))
    );
    ...
    for (const g of groups) {
      for (let i = 0; i < g.ids.length; i += BATCH) {
        batches.push({ ids: g.ids.slice(i, i + BATCH), embedding: g.embedding });
      }
    }
    const settled = await Promise.all(
      batches.map((b) => supabase.rpc('search_passages', rpcParams(b.ids, b.embedding)))
    );
  ...
  return {
    ...
    // Both notes can apply at once, and a caller that only ever reads `note`
    // must not have the seal explanation silently displace the partial-results
    // warning (or the reverse).
    ...((partialNote || sealedNote)
      ? { note: [sealedNote, partialNote].filter(Boolean).join(' ') }
      : {}),
...
// support, so the link is directly streamable by video/audio players and
// downloadable by connected agents (Antigravity/Gemini, Claude, ChatGPT).
// -----------------------------------------------------------------------------
export async function handleGetMedia(supabase, args) {
  ...
  return {
    ...
    note:
      ...
      'HTTP Range requests, so players can seek without downloading the whole ' +
      'file. The link expires; call get_media again for a fresh one. Do not ' +
...
export async function handleFileDocument(supabase, args, opts = {}) {
  ...
  const { processDocument, needsWorkerIngest, MEDIA_EXTENSIONS, OCRABLE_IMAGE_EXTENSIONS } = await import('./ingest-core.mjs');
  if (needsWorkerIngest(ext, fileBuf.length)) {
    ...
    return {
      ...
      status: 'queued',
      job_id,
      note: 'Large file — stored and queued for the background worker (no time limit). ' +
        'Not searchable yet; use check_ingest_status to follow progress. ' +
        'Typical documents finish within minutes.',
    };
  }

  //    Wire the Gemini hooks when a key is present: OCR so a scanned PDF files
  //    with real text + page coordinates, transcription so audio/video files
  //    as a timestamped transcript. processDocument marks the row 'ready'.
  try {
    let ocr;
    let transcribe;
    if (opts.googleApiKey && (ext === '.pdf' || OCRABLE_IMAGE_EXTENSIONS.includes(ext))) {
...
export async function handleIngestDocument(supabase, args) {
  ...
  const { data: doc, error } = await supabase.from('documents')
    .select('id, matterspace_id, source_filename, processing_status, storage_path')
    .eq('id', args.document_id)
    .maybeSingle();
...
export async function handleCheckIngestStatus(supabase, args) {
  ...
  if (args.document_id) {
    const { data: doc, error } = await supabase.from('documents')
      .select('id, source_filename, processing_status, processing_error, page_count')
      .eq('id', args.document_id)
      .maybeSingle();
    if (error) throw new Error(error.message);
    if (!doc) throw new Error('document not found or no access');
...
export async function handleAssembleDocuments(supabase, args, opts = {}) {
  ...
  const filed = await handleFileDocument(supabase, {
    ...
    filename,
    title: args.title,
    doc_type: args.doc_type || 'other',
    content: merged.bytes.toString('base64'),
    encoding: 'base64',
  }, opts);

  // Short-lived download link so the user can grab the merged PDF in their
  // browser even when the agent's own environment can't fetch storage URLs.
  let downloadUrl = null;
  const { data: newDoc } = await supabase
    .from('documents')
    .select('storage_path')
    .eq('id', filed.document_id)
    .maybeSingle();
  if (newDoc?.storage_path) {
    const { data: signed } = await supabase.storage
      .from('vault-documents')
      .createSignedUrl(newDoc.storage_path, 900);
  ...
  return {
    ...
    matter: filed.matter,
    filename,
    page_count: merged.ranges.length ? merged.ranges[merged.ranges.length - 1].to : 0,
    ...
    already_filed: filed.already_filed || undefined,
    download_url: downloadUrl,
    expires_in_seconds: downloadUrl ? 900 : undefined,
    note:
      'Merged PDF filed into the matter. Give download_url to the user as a ' +
      'clickable link — it works in their browser even if your own ' +
      'environment cannot fetch it. It expires; call get_media with the new ' +
...
async function fileGenerated(supabase, matterKey, filename, buf, { title, docType, ingest, opts = {} }) {
  if (ingest) {
    const filed = await handleFileDocument(supabase, {
      matter: matterKey,
      filename,
...
export async function handleEditPdf(supabase, args, opts = {}) {
  ...
  // Edited PDFs go through the full pipeline so the copy stays searchable.
  ...
  return {
    ...
    download_url: downloadUrl,
    expires_in_seconds: downloadUrl ? 900 : undefined,
    note: 'Edited copy filed alongside the original (which is unchanged). Give download_url to the user as a clickable link.',
  };
}
