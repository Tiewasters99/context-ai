...
export async function handleSearch(supabase, args, opts = {}) {
  ...
  if (matter) {
    const { data: rows, error } = await supabase.rpc('search_passages', rpcParams(matterIds));
    ...
    const BATCH = 12;
    const batches = [];
    for (let i = 0; i < matterIds.length; i += BATCH) batches.push(matterIds.slice(i, i + BATCH));
    const settled = await Promise.all(
      batches.map((ids) => supabase.rpc('search_passages', rpcParams(ids)))
    );
    ...
    if (failed > 0) {
      partialNote =
        `${failed} of ${batches.length} matter groups timed out and were skipped — ` +
        'results cover the rest. Re-run, or scope to a specific matter for full coverage there.';
    }
  }
