      ...
      properties: {
        ...
        doc_type: {
          ...
          description: 'Optional doc_type: transcript, deposition, exhibit, brief, expert_report, contract, correspondence, other. Default "other". ' +
            'A "brief" filed with a .md filename (the brief-format master.md dialect: ## parts, ### I. points, **A. sub-points**, [^n] footnotes) also opens in the Brief Desk as editable text.',
        },
      },
...
export async function handleFileDocument(supabase, args, opts = {}) {
  ...
  // 2b. A brief filed as Markdown opens in the Brief Desk as editable text
  //     (docs/specs/BRIEF-DESK-2026-09-26.md §3.5): a Claude Code drafting
  //     session files its first cut straight into the desk. The bytes above
  //     and the ingest below are unchanged; this only adds the editable body.
  //     Never fatal — without migration 100 the document is filed as before.
  if (docType === 'brief' && ext === '.md') {
    try {
      const { parse, serialize } = await import('./brief-md.mjs');
      const body = parse(fileBuf.toString('utf8'));
      const { error: bodyErr } = await supabase.from('draft_bodies').insert({
        document_id: doc.id, body, body_md: serialize(body), updated_by: createdBy,
      });
      if (bodyErr) console.warn('[file_document] brief body not created:', bodyErr.message);
    } catch (err) {
      console.warn('[file_document] brief body not created:', err?.message ?? err);
    }
  }
  ...
  if (needsWorkerIngest(ext, fileBuf.length)) {
    ...
    return {
      ...
      note: ext === '.zip'
        ...
        : OCRABLE_IMAGE_EXTENSIONS.includes(ext)
        ? 'Image — stored and queued for the background worker, which OCRs it (since 2026-09-26 the OCR no ' +
          'longer runs inline, so this reply carries no text). Not searchable yet; use check_ingest_status ' +
          'to follow progress. A picture with no words on it will finish as "image only".'
