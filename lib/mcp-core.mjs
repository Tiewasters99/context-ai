...
import { checkUpload, describeTextStatus } from './ingest-formats.mjs';
...
export async function handleFileDocument(supabase, args, opts = {}) {
  ...
  // zip: false — this path stores bytes as-is; only the web Vault expands
  // archives, so a .zip here would file as an unreadable blob.
  const refusal = checkUpload({ name: filename, size: fileBuf.length }, { zip: false });
...
export async function handleIngestDocument(supabase, args) {
  ...
  const { data: doc, error } = await supabase.from('documents')
    .select('id, matterspace_id, source_filename, processing_status, storage_path, text_status:metadata->>text_status')
    .eq('id', args.document_id)
    .maybeSingle();
  ...
  // 'ready' with a recorded text_status is stored-without-text, and a re-run
  // is exactly what a caller wants after wiring OCR/transcription — only a
  // ready document that is actually indexed has nothing to do.
