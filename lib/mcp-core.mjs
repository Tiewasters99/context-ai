...
export const TOOLS = [
  ...
  // ---------------------------------------------------------------------------
  // The task board (migration 085). Listed for every connector; only an AGENT
  // connection (Connections → Agents) has tasks — a user token gets a plain
  // one-line answer saying so. The loop: my_tasks → claim_task → work with the
  // normal tools inside the task's matter → ask_human if blocked → post_result.
  // ---------------------------------------------------------------------------
  {
    name: 'my_tasks',
    description:
      'AGENT CONNECTIONS ONLY. The task board: the tasks a person in Contextspaces has ' +
      'delegated to THIS agent, newest first. Start every session here, and poll it ' +
      'while you wait for an answer to ask_human. Default shows the live ones ' +
  ...
  },
  {
    name: 'claim_task',
    description:
      'AGENT CONNECTIONS ONLY. Take an open task from my_tasks before working on it ' +
  ...
  },
  {
    name: 'ask_human',
    description:
      'AGENT CONNECTIONS ONLY. Ask the person who delegated a task ONE clear question ' +
  ...
  },
  {
    name: 'post_result',
    description:
      'AGENT CONNECTIONS ONLY. Finish a task: store your result (the answer, summary or ' +
...
export const AGENT_TOOLS_ONLY_MESSAGE =
  'These tools are for agent connections. Add an agent under Connections → Agents.';
...
function agentTokenOf(opts = {}) {
  ...
  const scope = Array.isArray(a.matterScope)
    ...
    : [];
  return { id: a.id, userId: typeof a.userId === 'string' ? a.userId : null, matterScope: scope };
}
...
function agentDocArgs(args = {}) {
  ...
  if (Array.isArray(args.inserts)) {
    for (const ins of args.inserts) if (ins && typeof ins === 'object') take(ins.document_id);
  }
...
export async function enforceAgentScope(supabase, name, args = {}, allowed) {
  ...
  if (name === 'create_matter' && !(typeof args.parent === 'string' && args.parent)) {
    throw new AgentScopeError(
  ...
  for (const key of SEAL_MATTER_ARGS) {
    ...
    if (!m?.id || !allowed.has(m.id)) throw new AgentScopeError();
    touched += 1;
  }

  const docs = agentDocArgs(args);
  if (docs.malformed) throw new AgentScopeError();
  if (docs.ids.length) {
    const { data, error } = await supabase
    ...
    for (const id of docs.ids) {
      ...
      if (!mid || !allowed.has(mid)) throw new AgentScopeError();
    }
    touched += docs.ids.length;
  }

  if (name === 'get_passage' && typeof args.id === 'string' && args.id) {
    if (!UUID_RE.test(args.id)) throw new AgentScopeError();
    const { data, error } = await supabase
      .from('passages').select('matterspace_id').eq('id', args.id).maybeSingle();
    if (error) throw new Error(`agent scope check: ${error.message}`);
    if (!data?.matterspace_id || !allowed.has(data.matterspace_id)) throw new AgentScopeError();
    touched += 1;
  }

  if (touched === 0 && !AGENT_UNKEYED_OK.has(name)) throw new AgentScopeError();
}
...
async function dispatchWithSeal(supabase, name, args = {}, opts = {}) {
  ...
  if (AGENT_TASK_TOOLS.has(name) && !agent) return { error: AGENT_TOOLS_ONLY_MESSAGE };
  ...
  if (agent) {
    ...
    agentAllowed = await agentAllowedMatterIds(supabase, agent, agentSealed);
  ...
  const hidden = agent ? new AgentScopeHidden(agentAllowed, denied) : denied;
  if (agent) {
    opts = { ...opts, agentScope: { token: agent, allowed: agentAllowed, paused, sealed } };
  }
...
async function matterIdForTool(supabase, name, args = {}, opts = {}) {
  ...
  // The task board: an agent's call on one of ITS tasks belongs to that
  // task's matter Record. A task assigned to some other token is not looked
  // up further — the call is refused, and goes on the account chain.
  if (AGENT_TASK_TOOLS.has(name) && opts.agentToken?.id
      && typeof args.task_id === 'string' && UUID_RE.test(args.task_id)) {
    try {
      const { data } = await supabase
        .from('agent_tasks').select('matterspace_id, assigned_token_id')
        .eq('id', args.task_id).maybeSingle();
      if (data?.matterspace_id && data.assigned_token_id === opts.agentToken.id) {
...
// -----------------------------------------------------------------------------
// A person delegates a task to one of their agents from inside a matter; the
// agent, which can only call out, polls for it here. Every handler:
//   * answers only to the calling agent token — a task assigned to any other
//     token is "not found", never "not yours";
...
const TASK_NOT_FOUND = 'No task with that id is assigned to this agent. Call my_tasks for the list.';

/** The agent scope dispatchWithSeal attached, or a refusal. */
function taskScopeOf(opts = {}) {
  const s = opts.agentScope;
  if (!s?.token?.id || !(s.allowed instanceof Set)) throw new AgentScopeError(AGENT_TOOLS_ONLY_MESSAGE);
  return s;
}
...
async function taskMatterGate(supabase, scope, matterId) {
  ...
  if (!taskMatterVisible(scope, matterId)) throw new AgentScopeError();
}

/** 085 not applied yet reads as a sentence, not as a Postgres code. */
...
async function loadOwnTask(supabase, scope, taskId) {
  ...
  const { data, error } = await supabase
    .from('agent_tasks').select(TASK_COLS)
    .eq('id', taskId)
    .eq('assigned_token_id', scope.token.id)
    .maybeSingle();
  if (error) throw new Error(taskTableError(error));
  if (!data || data.assigned_token_id !== scope.token.id) throw new Error(TASK_NOT_FOUND);
...
async function logTaskEvent(supabase, scope, taskId, kind, body = null) {
  try {
    const actorUser = scope.token.userId ?? await resolveCallerId(supabase, {});
    ...
    .map((it) => {
      ...
      if (!hit) {
        return { kind: it.kind ?? null, id: it.id ?? null, available: false,
          note: 'Not available to this agent (outside its matters, sealed, or no longer there).' };
      }
...
export async function handleMyTasks(supabase, args = {}, opts = {}) {
  ...
  let q = supabase.from('agent_tasks').select(TASK_COLS).eq('assigned_token_id', scope.token.id);
  ...
  // A task in a matter the agent may not see right now (the grant changed, a
  // seal went on, the matter is paused) is left out entirely.
  const rows = (data ?? []).filter((t) => t.assigned_token_id === scope.token.id
  ...
  return {
    ...
    ...(rows.length === 0 ? { note: 'No tasks right now. Poll again later.' } : {}),
  };
}
...
async function checkResultRefs(supabase, scope, refs) {
  ...
  seen.forEach((it, i) => {
    if (!it.available) throw new AgentScopeError(`result_refs[${i}] is not in a matter this agent can see.`);
  });
  return clean;
}
