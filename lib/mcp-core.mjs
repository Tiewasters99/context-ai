...
import {
  sealedMatterIds, pausedMatterIds, matterPauseWithClient, aiPausedMessage, matterTierWithClient,
} from './ai-tier-policy.mjs';
import { record, recordCrossMatter, redactToolArgs, scrubArgValues } from './ledger.mjs';
import { pathInMatter, assertPathInMatter } from './storage-path.mjs';
import { repointDocumentJobs } from './job-scope.mjs';
...
async function dispatchTool(supabase, name, args = {}, opts = {}) {
  switch (name) {
    ...
    case 'get_media':            return handleGetMedia(supabase, args, opts);
    ...
    case 'move_document':        return handleMoveDocument(supabase, args, opts);
...
//
// S4a (migration 096): on a SEALED matter the URL is a copy leaving the seal,
// so it is handed over only once `file.opened {purpose:'read', via}` is in
// the matter's Record, with the actor the ledger already receives here. No
// caller reaches this on a sealed matter today — external connectors and
// agents are refused by enforceConnectorSeal / the agent grant before any
// handler runs, get_media is in neither the in-app assistant's ALLOWED_TOOLS
// nor api/sandbox.mjs's ALLOWED_ACTIONS, and the stdio server sets
// sealConnector. This is the rule for the day one of those changes.
// -----------------------------------------------------------------------------
async function documentMatterIsSealed(supabase, matterId) {
  if (!matterId) return false;
  const { data, error } = await supabase.rpc('effective_tier_is_sealed', { p_matter: matterId });
  if (!error && typeof data === 'boolean') return data;
  const code = String(error?.code ?? '');
  if (code === 'PGRST202' || code === '42883') {
    // 094 not pasted: the walk the pen gate uses.
    const tier = await matterTierWithClient(supabase, matterId);
    return tier === 'B' || tier === 'C';
  }
  throw new Error(`get_media: the seal could not be read${error?.message ? ` (${error.message})` : ''}`);
}

export async function handleGetMedia(supabase, args, opts = {}) {
  if (!args.document_id) throw new Error('document_id is required');

  const { data: doc, error } = await supabase
    .from('documents')
    .select('id, title, doc_type, source_filename, storage_path, file_size_bytes, matterspace_id')
  ...
  // 097: the object must be filed under this document's own matter — a
  // pointer row in one matter must never mint a URL for another's object
  // (a service-role caller, like the stdio server, would sign anything).
  if (!pathInMatter(doc.storage_path, doc.matterspace_id)) {
    throw new Error('get_media: this document’s stored file is filed under a different matter, so no link was issued.');
  }

  // Asked before the mint, so an unreadable seal mints nothing.
  const sealed = await documentMatterIsSealed(supabase, doc.matterspace_id);
  // 900 s on a sealed matter, whatever was asked for (S4a's ceiling).
  const expires = sealed
    ? 900
    : Math.min(Math.max(Math.trunc(args.expires_in ?? 900), 60), 3600);
  // After 096 the bucket refuses a user-scoped client a sealed object, so a
  // caller that may hand one out passes a service-role `opts.storageClient`
  // for the mint (the document row above was still read as the caller).
  // None does today; without one, a sealed mint is refused and nothing is
  // recorded, which is the fail-closed answer.
  const minter = sealed && opts.storageClient ? opts.storageClient : supabase;
  const { data: signed, error: signErr } = await minter.storage
  ...
  if (sealed) {
    const actor = opts.actor ?? null;
    const via = opts.sealConnector === true || actor?.kind === 'connector'
      ? 'connector'
      : (actor?.kind ?? 'unknown');
    const out = await record(supabase, {
      kind: 'file.opened',
      matterId: doc.matterspace_id,
      sessionId: actor?.session_id ?? opts.sessionId ?? null,
      actor,
      payload: { document_id: doc.id, title: doc.title ?? null, purpose: 'read', sealed: true, via },
    });
    // The URL just minted is dropped: a sealed matter's copy does not leave
    // without its row.
    if (!out?.ok) {
      throw new Error(
        'get_media: this document is in a sealed matter and its Record could not be written, ' +
        'so no link was issued. Try again in a moment.',
      );
    }
  }
...
export async function handleMoveDocument(supabase, args, opts = {}) {
  ...
  for (const id of ids) {
    ...
    if (doc.matterspace_id !== target.id) {
      ...
      if (doc.storage_path) {
        ...
        // 097: the stdio server runs this with the SERVICE ROLE, which moves
        // whatever key it is given — so only a path filed under the row's own
        // matter, in its exact shape (lib/storage-path.mjs).
  ...
  // Keep any queued/running/held ingest jobs pointed at the new matter so the
  // worker stamps fresh passages with the right scope. A member's own client
  // cannot update processing_jobs (032 has no UPDATE policy), so this uses the
  // caller's service-role `opts.jobClient` where it has one (api/sandbox.mjs,
  // api/mcp.mjs; the stdio server's client is the service role already), and
  // only for the documents that actually moved. Best-effort: the worker also
  // accepts a job whose document moved within the same tree (097 round 4).
  await repointDocumentJobs(opts.jobClient ?? supabase, moved.map((m) => m.document_id), target.id);
...
export async function handleCopyDocument(supabase, args, opts = {}) {
  ...
  for (const id of ids) {
    ...
    // 097: the copy below runs with the SERVICE ROLE on the stdio server;
    // refused before any row is written if the path is not the row's own.
    assertPathInMatter(src.storage_path, src.matterspace_id);
...
export async function handleAssembleDocuments(supabase, args, opts = {}) {
  ...
  for (const row of sources) {
    assertPathInMatter(row.storage_path, row.matterspace_id);   // 097, as in move_document
...
async function signDownloadUrl(supabase, documentId) {
  const { data: doc } = await supabase
    .from('documents')
    .select('storage_path, matterspace_id')
    .eq('id', documentId)
    .maybeSingle();
  if (!doc?.storage_path) return null;
  assertPathInMatter(doc.storage_path, doc.matterspace_id);   // 097, as in move_document
...
async function loadStoredPdf(supabase, documentId, label) {
  ...
  assertPathInMatter(src.storage_path, src.matterspace_id);   // 097, as in move_document
