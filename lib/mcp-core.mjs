...
async function fileGenerated(supabase, matterKey, filename, buf, { title, docType, ingest, pageCount, opts = {} }) {
  ...
  const { data: doc, error: insErr } = await supabase
    ...
    .insert({
      ...
      metadata: { text_status: TEXT_STATUS.GENERATED, text_status_at: new Date().toISOString() },
  ...
  for (let pos = 1; pos <= order.length + 1; pos += 1) {
    ...
    for (const ins of inserts) {
      if (ins.at === pos || (past && ins.at > order.length)) {
        for (const page of brought.get(ins)) { out.addPage(page); inserted += 1; }
      }
    }
    if (!past) out.addPage(own[pos - 1]);
  }
