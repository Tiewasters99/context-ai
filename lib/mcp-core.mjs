...
async function dispatchWithSeal(supabase, name, args = {}, opts = {}) {
  ...
  const sealed = opts.sealConnector
    ? await enforceConnectorSeal(supabase, name, args)
    : new Set();
  const hidden = paused.size === 0 ? sealed
...
export async function handleGrep(supabase, args, opts = {}) {
  ...
  let q = supabase
    .from('passages')
    // page_end is selected because a citation needs it: without it
    // formatCitation compared page_start to `undefined`, decided the passage
    // spanned a range, and printed "p. 1-undefined". `metadata` is NOT
    // selected here — this select can return 2000 candidate rows, and only the
    // handful that actually produce a match need it (one query, below).
    .select('id, document_id, matterspace_id, sequence_number, page_start, page_end, line_start, text')
