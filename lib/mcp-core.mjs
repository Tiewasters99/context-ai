...
export async function callTool(supabase, name, args = {}, opts = {}) {
  ...
  try {
    ...
    await recordToolInvoked(supabase, name, args, opts, { ok: true, ms: Date.now() - startedAt });
    return out;
  } catch (err) {
...
async function recordToolInvoked(supabase, name, args, opts, outcome) {
  try {
    ...
    if (!matterId) return;   // a matter-less call has no Record to go in
    ...
    await record(supabase, {
      ...
      payload: {
        tool: name,
        args,                                   // redacted in lib/ledger.mjs
        ...
        ms: outcome.ms ?? null,
        ...(outcome.error ? { error: String(outcome.error).slice(0, 200) } : {}),
      },
    });
  } catch {
