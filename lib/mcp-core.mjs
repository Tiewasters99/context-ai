...
export async function handleGetMatterState(supabase, args) {
  ...
  const { data: state, error: stErr } = await supabase
    .from('matter_state')
    .select('status, headline, next_action, next_action_owner, updated_at, updated_by')
    .eq('matterspace_id', matter.id)
    .maybeSingle();
...
export async function handleSetMatterState(supabase, args, opts = {}) {
  ...
  if (hasEdit) {
    const { data, error } = await supabase.rpc('set_matter_state', {
      ...
      p_next_action_owner: args.next_action_owner ?? null,
      p_updated_by: 'agent',
    });
...
export async function handleCheckIngestStatus(supabase, args) {
  ...
  return {
    matter: { id: matter.id, short_code: matter.short_code, name: matter.name },
    ready: readyCount ?? 0,
    still_processing: (notReady ?? []).filter((d) => d.processing_status !== 'error').map(docSummary),
    errors: (notReady ?? []).filter((d) => d.processing_status === 'error').map(docSummary),
