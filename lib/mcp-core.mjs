...
export async function handleFileDocument(supabase, args, opts = {}) {
  ...
  const { checkUpload } = await import('./ingest-formats.mjs');
  ...
  try {
    ...
    let stored = {};
    if (textStatus) {
      const { describeTextStatus } = await import('./ingest-formats.mjs');
...
export async function mergePdfBuffers(buffers) {
  ...
  // bundling a 2017 pdf.js) reports "Invalid PDF structure" on pdf-lib's
  // default output, so an assembled PDF would file as unreadable. Found by
  // the 2026-09-03 worker smoke test; same setting as its child fixtures.
