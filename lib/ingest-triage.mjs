...
export const TRIAGE = {
  ...
  media_too_long: {
    label: 'Recording too long for one transcription request',
    action: 'Gemini refused the whole recording at once (its 1,048,576-token input ceiling — about an hour of video). Since 2026-09-16 the worker cuts video as well as audio into 20-minute parts; re-run on a worker with that build.',
    retryable: true,
    severity: 'transient',
  },
  encoding_reject: {
  ...
  corrupt_file: {
    ...
    action: 'The file failed to parse. A PDF: node scripts/_repair-corrupt-pdfs.mjs --ids <id> re-reads it four ways and rewrites it through PyMuPDF if every parser refuses. A Word file: it was cut off in upload or is not a real .docx — open it in Word, Save As .docx, and upload again.',
    retryable: false,
    severity: 'blocking',
  },
  empty_file: {
  ...
  lock_file: {
    label: 'Office lock file, not a document',
    action: 'A "~$…" file is what Word writes beside an open document; it holds no content. Delete the row. Uploads of these are refused at selection since 2026-09-07.',
    retryable: false,
    severity: 'blocking',
  },
  web_capture: {
    label: 'Court website page, not the filing',
    action: 'The upload is a CM/ECF or PACER screen (Case Search, login) saved as HTML, not the document. Download the filing from PACER as a PDF, upload that, and delete this row.',
    retryable: false,
    severity: 'blocking',
  },
  retries_exhausted: {
  ...
  stored_archive: {
    label: 'Archive unpacked (children filed separately)',
    action: 'A .zip whose entries were filed as their own documents in a folder named after it (Phase 3). Check the children, not the archive.',
    retryable: false,
    severity: 'benign',
  },
  stored_media_no_transcript: {
    label: 'Recording stored without a transcript',
    action: 'Either the recording held no speech (silence / noise only — nothing to do), or no transcription hook was configured where this ingest ran (GOOGLE_API_KEY): set it and requeue if the recording should be searchable.',
    retryable: false,
    severity: 'benign',
  },
  stored_binary_asset: {
  ...
  stored_generated: {
    label: 'Generated deliverable stored',
    action: 'Built by a tool (a chart, a deck) and kept to open or download; the documents it was made from are the searchable copies.',
    retryable: false,
    severity: 'benign',
  },
  ...
  ocr_held_sealed: {
    ...
    action: 'The typed pages are indexed; the scanned pages were not sent out for OCR. The sealed route is AWS Textract in our own AWS account (Phase 4): provision it per docs/SEALED_OCR_SETUP.md, set TEXTRACT_AWS_ACCESS_KEY_ID / TEXTRACT_AWS_SECRET_ACCESS_KEY / TEXTRACT_AI_OPT_OUT_CONFIRMED on the worker, then node scripts/requeue-sealed-ocr.mjs. Until then, nothing to do.',
    retryable: false,
    severity: 'benign',
  },
  other: {
...
export function classifyError(msg = '') {
  ...
  //   · ingest-core's wording since 2026-09-07 (Phase 6) for a Word lock
  //     file, a court website page saved as HTML, and a .docx whose zip ends
  //     early — the three "unclassified" rows that kept the nightly suite red.
  if (m.includes('office lock file')) return 'lock_file';
  if (m.includes('court website page')) return 'web_capture';
  //   · and since 2026-09-23 for a Word 97–2003 .doc that word-extractor
  //     cannot open (damaged, or password-protected) — lib/doc-extract.mjs.
  if (m.includes('end of central directory') || m.includes('corrupted zip') || m.includes('not a real .docx') ||
      m.includes('could not read this word 97')) return 'corrupt_file';
  if (m.includes('statement timeout')) return 'db_timeout';
  //   · Gemini's answer to a whole hour of video sent in one request
  //     (2026-09-16): its own 1,048,576-token ceiling, not ours.
  if (m.includes('input token count exceeds')) return 'media_too_long';
...
export function attemptFailureNote(rawError, attempts, maxAttempts) {
  ...
  // The raw first line stays on the row in both notes. Without it, an
  // exhausted document carried only the class's boilerplate ("No rule matched.
  // Read the raw error…") and the raw error was gone from everywhere but the
  // worker's log — three times on 2026-09-16. With it, the monitor can still
  // classify the row later, once a rule for the cause exists.
  return {
    cls,
    exhausted: `${label}. ${t?.action ?? ''}${detail}`.trim(),
