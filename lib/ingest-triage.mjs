...
export function classifyError(msg = '') {
  ...
  if (m.includes('max_tokens_per_request') || m.includes('maximum input length') || m.includes('maximum request size')) return 'token_limit_400';
  // The recovery sweep's own wording (migration 058) for the two ways an
  // upload ends without a file to process, and for a document it gave up on.
  // They read well already; they just had no class, so every run listed them
  // as "unclassified" (2026-09-04: 8 of the 19).
  ...
  // ingest-core's own wording since 2026-09-04 for a scan it could not read
  // because no OCR hook was wired. Before that the same file failed as "no
  // passages extracted" and triage called it a benign photo.
...
export const TEXT_STATUS_CLASSES = Object.freeze(Object.values(TEXT_STATUS_CLASS));
