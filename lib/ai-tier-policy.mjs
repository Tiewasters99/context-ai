...
//   B — SEALED. Zero-retention pens: Claude via Bedrock in our own AWS
//       account (aws-bedrock — contractual ZDR under
//       data_retention_mode=none), or Kimi on Fireworks; first-party
//       Anthropic permitted only as a logged escalation (recorded, never
//       silent) on servers where the Bedrock pen is unprovisioned.
//   C — SILO. Local inference only; no cloud provider is permitted.
//       Until the Silo hardware exists, every cloud call is refused.
//
// ⚠ PARTLY RESOLVED 2026-08-27 (first flagged 2026-08-25) — Tier B's
// allowlist used to rest on two claims that vendor documentation did not
// support. Where they stand now:
//
...
//     and we have not obtained it. RESOLVED IN CODE: the sealed pen of
//     record is now frontier Claude via Bedrock (provider 'aws-bedrock',
//     Opus 5) — contractual zero retention in our own AWS account, verified
//     2026-08-27 against AWS's data-retention docs AND by live probe in our
//     account (Opus 5 and 4.8 both list 'none' in allowed_modes; under mode
//     none, Messages requests are never retained and the account
//     structurally cannot invoke a retention-requiring model). Once
//     the Bedrock key is configured, choosePen (lib/assistant-core.mjs)
//     never picks first-party Anthropic for a sealed matter. 'anthropic'
//     stays in the B set ONLY so the recorded-escalation fallback keeps
//     working while the Bedrock pen is unprovisioned — an escalation on
//     such a server is still "recorded, and also retained by the vendor
//     for a month". Removing it entirely (refusal instead of a retained
//     escalation) is a business call, deliberately not made in code.
//
...
//     unverified. "US-hosted" is therefore not established for this route.
//     With the Bedrock pen configured, this route is idle for sealed chat.
//
...
// not the model. Escalation to first-party Claude is recorded again whenever
// the sealed pen is not frontier Claude.
const TIER_PROVIDERS = {
  A: new Set(['anthropic', 'aws-bedrock', 'openai', 'google', 'xai', 'fireworks']),
  B: new Set(['aws-bedrock', 'fireworks', 'anthropic']),
  C: new Set(),
};
