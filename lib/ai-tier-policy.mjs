...
//
// ⚠ UNRESOLVED (checked 2026-08-25, during seal-pipes Phase B) — Tier B's
// allowlist below rests on two claims about providers that current vendor
// documentation does not support. Nothing here has been changed, because
// narrowing Tier B would take the sealed Assistant offline and that is a
// business decision, not a code one. But the claim in the line above should
// not be repeated to a client until this is settled:
//
//   * ANTHROPIC first-party (api.anthropic.com) retains inputs and outputs
//     for 30 days by default. Zero retention exists but is granted per
//     organization by their sales team — we have not obtained it. So a Tier-B
//     escalation to Anthropic today is "recorded, and also retained by the
//     vendor for a month", which is not what "sealed" implies. Routing Claude
//     through AWS Bedrock instead would give the ZDR this allowlist already
//     assumes, self-serve, with no sales call. Bedrock is zero-retention by
//     default and can be pinned with data_retention_mode=none and enforced
//     org-wide with an SCP.
//
