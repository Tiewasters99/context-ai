...
/**
 * A route can be fully configured — every variable present, the key valid —
 * and still have nothing listening. The SageMaker endpoint behind the sealed
 * route is the thing we switch OFF between sealed sessions to stop its hourly
 * meter (docs/SEALED_EMBEDDINGS_SETUP.md, "the switch you control"), and while
 * it is off, still coming up, or failed to provision, the runtime answers
 * 400 NO_SUCH_ENDPOINT. That is not a malformed request and not an outage to
 * retry through: it is the parked state, distinct from `routeReady` being
 * false (a route that was never set up). A route that has such a state
 * describes it with `unavailable(status, body) → reason | null`; embedBatch
 * and embedOne turn the reason into this error, and callers hold the vectors
 * (ingest) or search on words (retrieval) instead of failing.
 */
export class EmbedRouteUnavailableError extends Error {
  constructor(route, reason) {
    super(reason);
    this.name = 'EmbedRouteUnavailableError';
    this.code = 'embed_route_unavailable';
    this.route = route?.id ?? null;
    this.model = route?.model ?? null;
  }
}

/** True for the error above, however far it has been rethrown or wrapped. */
export function isEmbedRouteUnavailable(err) {
  return Boolean(err) && (err.code === 'embed_route_unavailable' || err.name === 'EmbedRouteUnavailableError');
}
...
export const ROUTES = {
  ...
  'voyage-4-sagemaker': {
    ...
    // The parked state, told apart from every other 400. The SageMaker runtime
    // answers exactly this while the endpoint is deleted (the hourly meter's
    // off switch), still creating, or failed to provision — observed on the
    // worker 2026-09-05:
    //   {"ErrorCode":"NO_SUCH_ENDPOINT","Message":"Endpoint voyage-4-embed of account … not found."}
    // Returns the reason a person reads, or null for any other response, so a
    // genuinely bad request still fails fast. Deliberately narrow.
    unavailable(status, body, { env = process.env } = {}) {
      if (status !== 400 || !body) return null;
      const s = String(body);
      if (!/NO_SUCH_ENDPOINT|\bEndpoint\b[^"]{0,120}\bnot found\b|\bnot in service\b/i.test(s)) return null;
      const name = present(env.SAGEMAKER_VOYAGE_ENDPOINT) ? env.SAGEMAKER_VOYAGE_ENDPOINT : 'voyage-4-embed';
      const region = present(env.AWS_REGION) ? env.AWS_REGION : 'us-east-1';
      return `The sealed embedding endpoint (SageMaker "${name}", ${region}, our own AWS account) is not in service — switched off, still starting, or never created.`;
    },
  },
};
