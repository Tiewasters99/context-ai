...
// record()) was being built in a parallel lane when this module was written,
// so the seam probes for ./ledger.mjs at runtime, uses it when it is there,
// and writes a structured server log line when it is not.
//
// 2026-09-20: it IS there — W1 landed. The probe stays (a deployment can still
// predate it, and the migration can be merged before it is pasted), but the
// call below is no longer a guess about a file that did not exist: it is
// lib/ledger.mjs's published contract, pinned, and its ANSWER is read.
//
...
// it, and where the ledger is there the same dialog says it is recorded.

let ledgerModule; // undefined = not probed yet · null = absent · object = loaded

// The import itself, behind a seam. A literal specifier so file tracing can
// follow it. Held in a variable so a harness can model the ABSENT case, which
// stopped being testable against the filesystem the day lib/ledger.mjs landed.
const REAL_LEDGER_IMPORT = () => import('./ledger.mjs');
let importLedger = REAL_LEDGER_IMPORT;
...
/**
 * Test seam: replace the IMPORT rather than its result, so "the module is not
 * there" can be exercised without deleting a file. Pass a function that
 * rejects with ERR_MODULE_NOT_FOUND for the absent case; pass nothing to
 * restore the real import. Either way the memoised probe is reset.
 */
export function __setLedgerImporterForTests(fn) {
  importLedger = typeof fn === 'function' ? fn : REAL_LEDGER_IMPORT;
  ledgerModule = undefined;
}

async function loadLedger() {
  if (ledgerModule !== undefined) return ledgerModule;
  try {
    // An absent module is an expected state, not an error.
    const mod = await importLedger();
...
export async function recordExport({ supabase, userId, tier, matterId, documentId, title, destination }) {
  ...
  if (ledger) {
    try {
      // PINNED to lib/ledger.mjs's contract (2026-09-20). What was here was
      // `if (ledger.record.length >= 2) record(supabase, event); else
      // record(event)` — a guess made while the file did not exist. It guessed
      // WRONG the moment it landed: `record(supabase, o = {})` has a default
      // second parameter, so Function.length is 1, and the export gate called
      // `record(event)`. That passes the event where the Supabase client
      // belongs; record() then returns {ok:false, 'no supabase client'}
      // WITHOUT throwing, and this function reported `recorded: true`. The
      // dialog and the banner would both have told a lawyer that a sealed
      // export was in the matter's Record while nothing had been written.
      //
      // So: the real shape, and the answer is READ rather than assumed.
      const out = await ledger.record(supabase, {
        kind: event.kind,
        matterId: event.matter_id,
        actor: { kind: 'user', ref: event.actor },
        // Metadata only — the same fields the log line carries. redact() in
        // lib/ledger.mjs is the floor under this, not a substitute for it.
        payload: {
          document_id: event.document_id,
          title: event.document_title,
          destination: event.destination,
          tier: event.tier,
          confirmed_leaving_seal: event.confirmed_leaving_seal,
          source: event.source,
        },
      });
      // A database with 064 merged but not pasted is the ABSENT case, not a
      // failure: the honest sentence is "exports are not written to any record
      // yet", which is exactly what `ledger_absent` says.
      if (out?.notDeployed) {
        logExport(event, { recorded: false, reason: 'ledger_absent' });
        return { recorded: false, reason: 'ledger_absent', event };
      }
      if (out && out.ok === false) {
        logExport(event, { recorded: false, reason: `ledger_error: ${out.error?.message ?? 'unknown'}` });
