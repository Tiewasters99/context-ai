...
/** The line the banner adds after a sealed export has actually happened. */
export function exportedNote({ destination, recorded }) {
  const d = normalizeDestination(destination);
  return recorded
    ? `This matter is sealed: a copy has left Contextspaces for ${d.label}, and it is recorded in the matter’s record.`
    : `This matter is sealed: a copy has left Contextspaces for ${d.label}. Exports are not written to any record yet, so nothing logs it.`;
}
...
export async function checkExport({ supabase, userId, documentId, destination, confirmed = false }) {
  ...
  return {
    ...
    recorded,
    note: exportedNote({ destination: d, recorded }),
  };
}
