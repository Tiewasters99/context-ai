    ...
    if (ocrConfigured && OCRABLE_IMAGE_EXTENSIONS.includes(lowerExt)) {
      ...
      if (ocrChars >= 40) {
        ...
        return await embedAndInsert(supabase, {
          ...
          metadataPatch: ocrRoute ? { ocr_route: ocrRoute } : null,
        });
      }
    }
...
async function embedAndInsert(supabase, { documentId, matterspace_id, passages, openaiApiKey, onProgress = () => {}, seal = { sealed: false }, priorMetadata = null, pendingOcr = null, metadataPatch = null }) {
  ...
  const hadReason = prior?.metadata &&
    ('text_status' in prior.metadata || 'text_status_at' in prior.metadata || 'ocr_pending' in prior.metadata ||
      'ocr_route' in prior.metadata || 'embedding_pending' in prior.metadata);
  if (hadReason || pendingOcr || metadataPatch || embeddingPending) {
    const { text_status: _ts, text_status_at: _at, ocr_pending: _op, ocr_route: _or, embedding_pending: _ep, ...rest } = prior?.metadata || {};
    ...
    if (transcript) {
      for (const c of transcript.chunks) {
        ...
        passages.push({
          ...
          // layer) are flagged on the passage so a cite can be checked.
