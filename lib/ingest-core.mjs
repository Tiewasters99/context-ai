...
function parseTranscriptPage(pageText) {
  ...
  // The witness call, one-line form: "JOHANNE SAINT-FLEUR, having been first
  // duly sworn" (periods allowed — "FELIX EZEKWE, M.D., was sworn").
  const witnessCallRE = /\b([A-Z][A-Z'\-\. ]+?),\s+(?:having (?:first )?been|was)\b/;
  // …and the two-line form reporters use: the name alone on one line
  // ("FELIX EZEKWE, M.D.,"), the oath on the next ("having first been duly
  // sworn, testified as follows:"). Both lines are headings; the name line
  // carries the witness.
  const nameLineRE = /^([A-Z][A-Z'\-\.]*(?:\s+[A-Z][A-Z'\-\.]*){1,4})(?:,\s*(?:M\.D\.|PH\.D\.|D\.O\.|R\.N\.|ESQ\.|JR\.|SR\.|III))?,?\s*$/;
  const callLineRE = /\b(?:having (?:first )?been(?: first)? duly sworn|duly sworn|testified as follows|examined and testified|was sworn)\b/i;
  ...
  // Until 2026-09-10 the gap was `\s{1,6}`, which matched the newline when a
  // numbered line was blank (" 5\n 6  Q. Why…"), so line 6's text was read as
  // line 5's, prefixed by "6", the Q./A. test never fired, and the page fell
  // to prose. Reporter transcripts routinely leave numbered lines blank
  // (found by the demo-corpus session on its rendered transcripts, 2026-09-09).
  ...
  // Reporter PDFs that draw the line numbers OUTSIDE the text layer (Veritext's
  // exports — every Fleming deposition, 2026-09-10): no digit prefixes at all,
  // but a page of 20–30 text lines with Q./A. markers. Number the text lines
  // by their position on the page and say so on every chunk
  // (metadata.line_numbers = 'inferred'), so a cite reads "41:13-16" and the
  // reader knows the numbers came from line order, not the reporter's margin.
  // A page with blank numbered lines in the original will be off by those
  // lines; the flag is what lets a citation be checked before it is used.
  let inferred = false;
  if (lines.length < 8) {
    const nonEmpty = pageText.split('\n').map((l) => l.trim()).filter((l) => l.length > 0);
    const markers = nonEmpty.filter((l) =>
      /^(Q\.|A\.)\s/.test(l) || /^BY\s+(MR\.|MRS\.|MS\.|DR\.)/.test(l) || headerRE.test(l) || witnessCallRE.test(l)).length;
    if (nonEmpty.length < 15 || nonEmpty.length > 32 || markers < 2) return null;
    lines.length = 0;
    nonEmpty.forEach((text, i) => lines.push({ lineNum: i + 1, text }));
    inferred = true;
  }
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    // Two-line call: this line is a bare name and the next non-empty line
    // takes the oath.
    const nextText = (lines[i + 1]?.text || lines[i + 2]?.text || '');
    const nameBeforeCall = !witnessCall && nameLineRE.test(text) && callLineRE.test(nextText);
    if (nameBeforeCall) witnessName = nameLineRE.exec(text)[1].trim();
    // "THEREUPON," / "WHEREUPON," is the reporter's cue before the call.
    const isCall = !!witnessCall || nameBeforeCall || callLineRE.test(text) || /^(THEREUPON|WHEREUPON),?$/i.test(text);
