...
import { detectPrintedPages, printedPageFor, verdictForDocument, METHOD } from './transcript-pages.mjs';
...
function parseTranscriptPage(pageText, { relaxed = false } = {}) {
  ...
  if (condensedMarks >= 2) {
    ...
    if (lines.length < 8) {
      ...
      lines = nonEmpty.map((text, i) => ({ lineNum: i + 1, text }));
      inferred = true;
    }
  }
