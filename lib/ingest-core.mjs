...
export async function processDocument(supabase, options) {
  ...
  if (lowerExt === '.epub') {
    ...
    // No text layer at all, and no OCR hook: the honest outcome is a failure
    // with a cause, not the old "no passages extracted" (which triage read as
    // a benign photo) and not a silent store. The message names the fix; the
    // triage table maps it to ocr_needed. Deliberately narrower than
    // looksScanned: a text-LIGHT PDF (a one-page exhibit tab, a cover sheet)
    // still has real text, and without OCR available indexing that text is
    // better than refusing the file — per-page OCR of the rest is Phase 2.
    if (extractedChars < 40 && lowerExt === '.pdf' && typeof ocr !== 'function') {
...
async function embedAndInsert(supabase, { documentId, matterspace_id, passages, openaiApiKey, onProgress = () => {}, seal = { sealed: false }, priorMetadata = null }) {
  ...
  if (passages.length === 0) {
    ...
    return await markStoredWithoutText(supabase, documentId, TEXT_STATUS.NO_TEXT, {
      onProgress, message: 'Stored — the file opened but held no text to index',
    });
  }
  ...
  if (prior?.metadata && ('text_status' in prior.metadata || 'text_status_at' in prior.metadata)) {
    const { text_status: _ts, text_status_at: _at, ...rest } = prior.metadata;
    readyUpdate.metadata = rest;
  }
  await supabase
    .from('documents')
    .update(readyUpdate)
...
async function markStoredWithoutText(supabase, documentId, textStatus, { pageCount = null, message, onProgress = () => {}, metadata = {} } = {}) {
  ...
  return { passageCount: 0, textStatus };
}
