...
export const SUPPORTED_EXTENSIONS = [
  ...
  '.eml',
  // Images: JPEG/PNG are OCR'd through the injected `ocr` hook (a scanned
  // page saved as a .jpg — vFlat, phone camera — is a document and must come
  // out searchable). Other image formats store-and-display.
...
// through the same `ocr` hook as scanned PDFs. Exported so every ingestion
// entry point wires the hook for these exactly as it does for '.pdf'.
//
// TIFF is on this list despite pdf-lib not embedding it, because it has to be:
// the 2026-08-22 ingestion audit found 5,782 stored TIFFs with zero passages
// between them, 5,775 of which are `BMC_00xxxx.tif` — Bryn Mawr's entire Bates
// production in the live DeCamara matter, filed, marked `ready`, and completely
// unsearchable. A Bates-stamped production page is the most document-like thing
// in the vault; storing it as a picture was always the wrong answer.
// imageToPdf() transcodes TIFF to PNG with sharp first, and handles multi-page
// TIFFs — see the note there.
...
export async function processDocument(supabase, options) {
  ...
  if (IMAGE_EXTENSIONS.includes(lowerExt)) {
    ...
    if (typeof ocr === 'function' && OCRABLE_IMAGE_EXTENSIONS.includes(lowerExt)) {
      ...
      let pages = null;
      try {
        pages = await ocr(await imageToPdf(fileBuf, lowerExt));
      } catch (err) {
        // Undecodable/corrupt image or OCR outage: fall through to
        // store-and-display rather than stranding the doc in error.
        onProgress({ stage: 'extracting', message: `Image OCR failed (${err.message}) — storing as image` });
      }
      ...
      if (ocrChars >= 40) {
        ...
        return await embedAndInsert(supabase, {
          documentId, matterspace_id, passages, openaiApiKey, onProgress, seal,
        });
      }
    }
    await supabase
      ...
      .update({
        ...
        // A successful run must erase the record of any earlier failed
        // attempt — a stale error alongside 'ready' reads as broken.
  ...
  if (MEDIA_EXTENSIONS.includes(lowerExt)) {
    ...
    if (typeof transcribe !== 'function') {
      await supabase
        .from('documents')
        .update({ processing_status: 'ready', processing_error: null, ingested_at: new Date().toISOString() })
        .eq('id', documentId);
  ...
  if (!STRUCTURED_EXTS.includes(lowerExt) && looksBinary(fileBuf)) {
    await supabase
      ...
      .update({ processing_status: 'ready', processing_error: null, ingested_at: new Date().toISOString() })
      .eq('id', documentId);
    onProgress({ stage: 'ready', message: 'Binary file stored (no text extraction for this format)' });
    return { passageCount: 0 };
  }
  ...
  if (lowerExt === '.epub') {
    ...
    // text; a scanned one yields ~nothing from pdf-parse — EXCEPT the
    // CM/ECF header stamp, which the court's filing system overlays as
    // real digital text (~100 chars/page) on top of the scanned images.
    // "Found some text" is therefore NOT "readable document": a scanned
    // verdict form ingested as stamp-lines only (2026-08-10). A
    // born-digital page of legal text runs thousands of characters; under
    // ~200/page on average, treat the PDF as scanned and OCR it. A false
    // positive just costs one OCR pass that usually improves the text.
    const extractedChars = pages.reduce((s, p) => s + (p.text || '').trim().length, 0);
    const looksScanned = lowerExt === '.pdf' && extractedChars < Math.max(40, pages.length * 200);
    // Sealed, and the only way to read this PDF is to send its pages out.
    // (A born-digital PDF never reaches here: its text layer extracted
    // locally, so it goes on to be chunked and text-indexed as normal.)
    if (looksScanned && seal.sealed && typeof ocr === 'function') refusePipe('ocr');
    if (looksScanned && typeof ocr === 'function') {
      ...
      // OCR ran and still found ~no words: a photo/diagram-only PDF (injury
      // photos, exhibit images). That's a valid document to view, not a
      // failure — store-and-display like an image instead of erroring.
      const ocrChars = pages.reduce((s, p) => s + (p.text || '').trim().length, 0);
      if (ocrChars < 40) {
        await supabase
          .from('documents')
          ...
          .eq('id', documentId);
        onProgress({ stage: 'ready', message: 'Image-only document stored (no readable text)' });
        return { passageCount: 0 };
      }
    }
...
async function embedAndInsert(supabase, { documentId, matterspace_id, passages, openaiApiKey, onProgress = () => {}, seal = { sealed: false } }) {
  ...
  //
  // Sealed matters skip this entirely: every passage below is inserted with a
  // null embedding, which costs them stage A of migration 056's search (the
  // meaning-based half) and nothing else. The text, the tsvector, the
  // citations and the page:line coordinates are all produced locally, so the
  // document is genuinely searchable the moment it lands — and Phase B finds
  // exactly what it has to backfill with `where embedding is null`.
  let embedded = 0;
  if (!seal.sealed) {
    ...
    onProgress({
      stage: 'embedding',
      message: `Sealed matter — indexing ${passages.length} passages for text search, no embeddings sent out`,
    });
  }
  ...
  for (let i = 0; i < passages.length; i += INSERT_CHUNK) {
    const batch = passages.slice(i, i + INSERT_CHUNK).map((p) => ({
      ...
      // Null on the sealed path. embedding_model keeps its column default
      // (text-embedding-3-small) rather than being blanked: it names the
      // space this passage BELONGS to, which is what search_passages filters
      // its full-text stage on — write anything else and a sealed matter
      // stops matching its own text.
  ...
  onProgress({
    stage: 'ready',
    message: seal.sealed
      ? `${passages.length} passages indexed for text search (sealed matter — nothing sent out)`
      : `${passages.length} passages indexed`,
  });

  return { passageCount: passages.length, embedded: !seal.sealed };
}
...
export async function embedBatch(apiKey, texts, { maxRetries = 6, limiter = undefined } = {}) {
  ...
  for (;;) {
    ...
    if (parseOversizedRequest(res.status, body)) {
      if (texts.length > 1) {
        ...
        const head = await embedBatch(apiKey, texts.slice(0, mid), { maxRetries, limiter });
        const tail = await embedBatch(apiKey, texts.slice(mid), { maxRetries, limiter });
