...
async function embedAndInsert(supabase, { documentId, matterspace_id, passages, openaiApiKey, onProgress = () => {} }) {
  ...
  // grows — a chunk size that clears the statement timeout today won't
  // forever (2,047pp Fleming Sixth Production, 2026-07-22). On a statement
  // timeout, halve the chunk and retry instead of failing the doc; a
  // cancelled statement is rolled back, so re-inserting the same rows is safe.
