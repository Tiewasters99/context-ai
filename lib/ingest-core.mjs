...
import { SealedPipeError, matterSeal, assertAiNotPaused } from './seal-pipes.mjs';
import { ROUTES, resolveRoute, EmbedRouteUnavailableError, isEmbedRouteUnavailable } from './embed-routes.mjs';
import { contentChars, isStampOnlyText } from './court-stamps.mjs';
import { isUnreadableText } from './text-quality.mjs';
import { detectPrintedPages, printedPageFor, verdictForDocument, lineColumnBounds, METHOD } from './transcript-pages.mjs';
import { westlawStarPages, westlawPageMeta } from './westlaw-pages.mjs';
import { bluebookCitesFor, westlawCaseSummary } from './bluebook.mjs';
...
//
// The ceiling per kind of file, and the arithmetic behind it (2026-09-20)
// ----------------------------------------------------------------------
// Until now everything that was not a PDF, a recording or a TIFF ran inline up
// to 20 MB, and a 15 MB .docx or .pptx is the document that pays for it: it
// sits under every threshold, runs on api/ingest's 60 s budget (vercel.json),
// and a kill part-way leaves a row with NO job to reap. The worker's idle
// sweep does find it — recover_stranded_documents, case 2a — but only after
// RECOVER_IDLE_MINUTES (15) and only while the worker is up, which is a long
// spinner for someone's first upload.
//
// The long pole inside the budget is not extraction, it is EMBEDDING. A
// passage is MAX_PASSAGE_WORDS (500) words ≈ 2,500 characters; a request
// carries EMBEDDING_BATCH (96) of them and takes roughly 1.5–3 s. So N bytes
// of extractable text costs about N / 2,500 passages ≈ N / 240,000 requests:
//
//     2 MB of text  →   ~840 passages →  ~9 requests →  15–30 s
//     4 MB of text  → ~1,700 passages → ~18 requests →  30–60 s   ← the kill zone
//
// and that is before the file is downloaded from storage, extracted, chunked
// and inserted. 2 MB is therefore the ceiling for a format whose BYTES ARE
// ITS TEXT, and 1 MB for the zip-compressed Office and e-book containers,
// whose bytes badly understate it — a 1 MB .docx of prose unzips to several
// megabytes of text, and nothing can measure that before the download.
//
// Erring towards the worker costs one poll cycle of queue latency (~5 s). The
// opposite error costs a document. So every uncertain case goes to the queue,
// where a job row exists, the monitor can see it, and retries are bounded.
const MB = 1024 * 1024;
export const INLINE_MAX_BYTES = Object.freeze({
  // Bytes are (mostly) the text: .txt, .md, .csv, .rtf, .eml, .fountain, code.
  text: 2 * MB,
  // Zip-compressed text containers: .docx, .pptx, .xlsx, .epub.
  packed: 1 * MB,
  // Unchanged. A PDF below this that needs OCR on any page is routed by
  // planPdfOcr instead, which is the test size never could make.
  pdf: 10 * MB,
  // Store-and-display formats only (.gif/.webp/.svg/.bmp): the OCR-able ones
  // (.jpg/.jpeg/.png) always go to the worker — see needsWorkerIngest.
  image: 8 * MB,
  // Unchanged: Gemini takes the whole recording inline below this.
  media: 12 * MB,
  // 3D/CAD assets are never extracted — only downloaded and marked stored.
  binary: 20 * MB,
  // An unrecognised extension is whatever sniffExtension makes of it, so it
  // is priced as the worst of those: text.
  other: 2 * MB,
});

const PACKED_TEXT_EXTENSIONS = ['.docx', '.pptx', '.xlsx', '.epub'];

// Which ceiling in INLINE_MAX_BYTES applies to `ext`. Exported for the
// routing test; every surface gets the answer through needsWorkerIngest.
export function inlineBudgetClass(ext) {
  const e = String(ext || '').toLowerCase();
  if (e === '.pdf') return 'pdf';
  if (MEDIA_EXTENSIONS.includes(e)) return 'media';
  if (IMAGE_EXTENSIONS.includes(e)) return 'image';
  if (BINARY_ASSET_EXTENSIONS.includes(e)) return 'binary';
  if (PACKED_TEXT_EXTENSIONS.includes(e)) return 'packed';
  if (SUPPORTED_EXTENSIONS.includes(e) || PLAIN_TEXT_EXTENSIONS.includes(e)) return 'text';
  return 'other';
}
...
export function needsWorkerIngest(ext, sizeBytes) {
  ...
  // Unconditional, whatever the size: these cannot run on a Vercel function
  // at all (no ffmpeg, no proven sharp) or fan out into N uploads and N jobs.
  if (e === '.wma') return true;                     // ffmpeg transcode
  if (e === '.zip') return true;                     // unpacked at ingest (Phase 3)
  if (e === '.tif' || e === '.tiff') return true;    // sharp transcode + OCR — see above
  // Every OCR-able image, whatever the size (2026-09-26). One OCR call is
  // 20–60 s of Gemini round trips on a 60 s budget — and the caller is a
  // browser holding the request open, so the Vault's upload loop waited
  // that long for EACH picture: forty-seven concept renders for a screenplay
  // landed one every 30–60 s. Queued, the request answers in a second, the
  // next file starts, and the OCR runs on the worker where a slow window
  // cannot strand the row. Formats that are not OCR'd (.gif/.webp/.svg/.bmp)
  // are store-and-display — no provider call — and keep the inline path.
  if (OCRABLE_IMAGE_EXTENSIONS.includes(e)) return true;
  // Anything huge, whatever it is: the download alone can spend the budget.
  if (size > 20 * MB) return true;
  return size > INLINE_MAX_BYTES[inlineBudgetClass(e)];
}
...
export function sniffExtension(buf) {
  ...
  if (b.slice(4, 8).toString('latin1') === 'ftyp') {
    ...
    // HEIF/HEIC and AVIF are ISO base media too, and share the ftyp box with
    // video. Before this guard an iPhone photo that arrived without an
    // extension — an email attachment, an export named "image" — sniffed as
    // .mp4 and was sent to the A/V TRANSCRIBER: a paid call on a still
    // picture, answered with nothing. They are refused at selection by name
    // (ingest-formats' UNSUPPORTED_HINTS) because no decoder here reads HEVC;
    // returning null makes the nameless copy take the same path — stored with
    // a recorded reason — instead of a worse one.
    if (/^(heic|heix|heim|heis|hevc|hevx|mif1|msf1|avif|avis)$/.test(brand.toLowerCase())) return null;
  ...
  // A web page saved without an extension ("Order reinstating appeal", from
  // a browser's Save As on PACER): before 2026-09-24 it went to the plain-text
  // fallback and its HTML source was indexed. As .html it gets the tag-free
  // reader, and a court site's search/login screen is refused with the reason.
  const head = b.subarray(0, 1024).toString('latin1').replace(/^\xEF\xBB\xBF/, '').trimStart().toLowerCase();
...
export async function processDocument(supabase, options) {
  const {
    ...
    openaiApiKey,
    onProgress = () => {},
    // Optional OCR for scanned pages (no text layer). Two shapes are accepted:
    //   * a bare hook, (fileBuf) => [{pageNumber,text}] — every caller before
    //     Phase 4 and the tests' stubs; treated as an UNSEALED provider;
    //   * a provider from lib/ocr-routes.mjs (makeOcrProvider) with
    //     plan(tier) → { routes, reason } and run(buf, { tier }) → { pages,
    //     route }, which picks the route the matter's tier allows (Textract
    //     inside a SecureSpace, Gemini/Anthropic outside), falls back within
    //     the tier, and reports which route read the pages and what it cost.
    // Kept injectable so ingest-core stays provider-agnostic: nothing here
    // names a provider, and the seal check asks only "does this tier have a
    // route?".
    ocr = null,
  ...
  const { data: doc, error: docErr } = await supabase
    .from('documents')
    .select('id, matterspace_id, witness_name, metadata, source_filename')
    .eq('id', documentId)
    .single();
  if (docErr) throw new Error(`processDocument lookup: ${docErr.message}`);
  // "~$…" is the lock file Word writes beside an open document: the editor's
  // name, never content. Refused at selection since 2026-09-07 (checkUpload);
  // one that reaches here anyway — a folder dragged in before that, a bulk
  // import — fails with the reason instead of as an unreadable .docx.
  const baseName = String(doc.source_filename || '').split(/[\\/]/).pop() || '';
  if (baseName.startsWith('~$')) {
    const reason = `"${baseName}" is a temporary Office lock file, not a document — Word writes it beside an open file and it holds no content. Delete it and upload the real document.`;
    await supabase.from('documents').update({ processing_status: 'error', processing_error: reason }).eq('id', documentId);
  ...
  // The AI pause (migration 070). Unlike the seal, which lets local work
  // through, a pause stops the document here — before extraction — because
  // "pause all AI on this matter" is a person saying stop, and finishing the
  // document with an empty index is the 2026-08-22 lie migration 060 exists to
  // prevent. Nothing is dropped: the caller parks it in 'held' with the
  // pause's own sentence (isSealedPipeError answers true for it), and
  // resuming requeues exactly these rows.
  await assertAiNotPaused(supabase, matterspace_id, { pipe: 'embeddings', pause: seal.pause });

  // The OCR question, answered once (Phase 4): which routes may read THIS
  // matter's scanned pages? A sealed matter with no ready sealed route is
  // held, and the hold names what is missing; an unsealed matter with no
  // route at all records its pages as awaiting OCR with the same reason.
  const ocrProvider = normalizeOcrHook(ocr);
  const ocrPlan = ocrProvider ? ocrProvider.plan(seal.tier ?? 'A') : { routes: [], reason: 'OCR is not configured where this ran.' };
  const ocrConfigured = ocrPlan.routes.length > 0;
  const ocrHeldBySeal = seal.sealed && !ocrConfigured;
  const runOcr = (buf) => ocrProvider.run(buf, { tier: seal.tier ?? 'A', onProgress });
  // Which route read the pages, for documents.metadata.ocr_route.
  let ocrRoute = null;
  ...
  } else if (lowerExt === '.doc') {
    // A .doc is a Word binary, or RTF / .docx / PDF / HTML under that name
    // (Westlaw's "Word" download is one of these): read it as what it is.
    const { docFormat } = await import('./doc-extract.mjs');
    const actual = docFormat(fileBuf);
    if (actual && actual !== '.doc') {
      onProgress({ stage: 'extracting', message: `File is ${actual} under a .doc name — extracting by content` });
      lowerExt = actual;
    }
  ...
  if (IMAGE_EXTENSIONS.includes(lowerExt)) {
    ...
    if (ocrHeldBySeal && ocrProvider && OCRABLE_IMAGE_EXTENSIONS.includes(lowerExt)) {
      refusePipe('ocr');
    }
    if (ocrConfigured && OCRABLE_IMAGE_EXTENSIONS.includes(lowerExt)) {
      await setStatus(supabase, documentId, 'extracting');
      ...
      if (asPdf) {
        try {
          const out = await runOcr(asPdf);
          pages = out.pages;
          ocrRoute = out.route || null;
        } catch (err) {
      ...
      if (ocrChars >= 40) {
        ...
        const imageReport = {};
        const passages = chunkPages(pages, { witness_name: effectiveWitness, report: imageReport });
        // A transcript can arrive as a multi-page scan too, and its cites
        // carry the same risk — record the verdict on the row either way.
        const imagePrinted = imageReport.printedPages;
        return await embedAndInsert(supabase, {
          documentId, matterspace_id, passages, openaiApiKey, onProgress, seal, priorMetadata,
          metadataPatch: (ocrRoute || imagePrinted?.is_transcript)
            ? {
              ...(ocrRoute ? { ocr_route: ocrRoute } : {}),
              ...(imagePrinted?.is_transcript ? { transcript_pages: verdictForDocument(imagePrinted) } : {}),
            }
            : null,
        });
      }
    ...
    return await markStoredWithoutText(supabase, documentId, TEXT_STATUS.IMAGE_ONLY, {
      onProgress,
      message: OCRABLE_IMAGE_EXTENSIONS.includes(lowerExt) && ocrConfigured
  ...
  if (MEDIA_EXTENSIONS.includes(lowerExt)) {
    ...
    // Nothing spoken — a silent or noise-only recording. Gemini answers those
    // with "[silence]" / "[background noise]" markers, which transcribeMedia
    // strips down to no segments at all. Kept, viewable, honestly labelled:
    // "ready" over one passage reading "[00:00] [silence]" counted as
    // searchable until 2026-09-06 (the nightly suite caught it).
    if (!segs || segs.length === 0) {
      return await markStoredWithoutText(supabase, documentId, TEXT_STATUS.MEDIA_NO_TRANSCRIPT, {
        onProgress, message: 'Recording stored — no speech found to transcribe',
      });
    }
  ...
  const STRUCTURED_EXTS = ['.pdf', '.docx', '.doc', '.xlsx', '.pptx', '.eml', '.epub', '.fountain', '.rtf'];
  ...
  if (lowerExt === '.epub') {
    ...
    //
    // Stamps are not content (2026-09-18). The header stamp is real text on an
    // otherwise unread scan, so a character count that includes it calls the
    // scan "text": the decisions below then indexed the stamp and marked the
    // document ready — ECF 53 and ECF 58 in decamara, two 2-page orders stored
    // as one stamp line each, found four months later. contentChars counts a
    // page that is stamps and nothing else as zero; a typed page counts in
    // full. Which pages go to OCR is still pagesNeedingOcr's decision alone.
    const extractedChars = contentChars(pages);
    let pendingOcr = null;
    // What this run did with the PDF's pages, kept on the row
    // (metadata.ingest_outcome) — see ingestOutcome.
    let outcome = null;
    if (lowerExt === '.pdf') {
      const ocrTargets = await pagesNeedingOcr(fileBuf, pages);
      const textLayerPages = pages;
      const record = (ocr, extra) => ingestOutcome(textLayerPages, ocrTargets, ocr, extra);
      outcome = record('not_needed');
      if (ocrTargets.length) {
        ...
        if (extractedChars < 40 && ocrHeldBySeal && ocrProvider) refusePipe('ocr');
        if (extractedChars < 40 && !ocrConfigured) {
          const reason = `Scanned PDF — OCR not configured (${pages.length} page(s), ${extractedChars} chars of text beyond any filing stamp). ` +
            `${ocrPlan.reason} Set GOOGLE_API_KEY or ANTHROPIC_API_KEY where this ingest runs, then Retry.`;
          await supabase
            .from('documents')
        ...
        if (ocrHeldBySeal) {
          // Mixed document in a SecureSpace with no ready sealed route: the
          // typed pages are read locally and indexed; the scanned pages would
          // have to leave, so they are recorded as not read, never retried by
          // the sweep, and the record says what is missing (Phase 4: the
          // Textract route — docs/SEALED_OCR_SETUP.md). Once it is configured,
          // scripts/requeue-sealed-ocr.mjs re-runs these.
          pendingOcr = buildOcrPending(priorMetadata, {
            pages: ocrTargets, pageCount: pages.length, held: true,
            reason: `SecureSpace seal — ${ocrTargets.length} scanned page(s) were not sent out for OCR. ${ocrPlan.reason}`,
          });
          outcome = record('held');
          onProgress({ stage: 'extracting', message: `Sealed matter — ${ocrTargets.length} scanned page(s) left unread (${firstLine(ocrPlan.reason)}); indexing the ${pages.length - ocrTargets.length} typed page(s)` });
        } else if (!ocrConfigured) {
          pendingOcr = buildOcrPending(priorMetadata, {
            pages: ocrTargets, pageCount: pages.length,
            reason: ocrPlan.reason,
          });
          outcome = record('not_configured');
          ...
          try {
            const merged = await ocrPdfPages(fileBuf, pages, ocrTargets, runOcr, onProgress);
            // OCR answered and read NOTHING on a filing whose every scanned
            // page carries a court stamp. A filed order is not a photograph:
            // the likelier story is an answer without the page markers, which
            // parseDelimited turns into '' for every page, and the merge then
            // keeps each stamp — the stamp-only document all over again. Treat
            // it as the OCR failing: the pages are recorded as awaiting OCR,
            // the sweep retries them, and if it never reads them a person
            // sees why (ocr_exhausted), instead of "image-only, benign".
            // "Every scanned page" means every target that is not blank: a
            // stamped order's blank back page has no stamp and no text, and
            // must not tip the document into image-only.
            const own = ocrTargets.map((n) => pages[n - 1]?.text || '');
            if (merged.filled === 0 && own.some(isStampOnlyText) && own.every((t) => !t.trim() || isStampOnlyText(t))) {
              throw new Error(`OCR returned no text for any of the ${ocrTargets.length} scanned page(s), though each carries a court filing stamp`);
            }
            pages = merged.pages;
            ocrRoute = merged.route || null;
            outcome = record('read', { noText: merged.stillEmpty });
            if (!wholeDoc) {
            ...
            if (extractedChars < 40) {
              ...
              return await markStoredWithoutText(supabase, documentId, TEXT_STATUS.OCR_PENDING, {
                ...
                metadata: { ocr_pending: pendingOcr, ingest_outcome: outcome },
      ...
      // that case was recorded above and is not "OCR found nothing". A
      // stamp is not words here either (contentChars).
      const afterOcrChars = contentChars(pages);
      if (afterOcrChars < 40 && !pendingOcr) {
        return await markStoredWithoutText(supabase, documentId, TEXT_STATUS.IMAGE_ONLY, {
          pageCount: pages.length, onProgress,
          metadata: { ingest_outcome: outcome },
    ...
    const chunkReport = {};
    passages = chunkPages(pages, { witness_name: effectiveWitness, report: chunkReport });
    // Whether this document's citations name the reporter's printed page or
    // the PDF's page index — recorded on the row so an audit can list every
    // transcript still cited by index without re-parsing a single file.
    const printedPages = chunkReport.printedPages;
    if (printedPages?.is_transcript) {
      metadataPatch = { ...(metadataPatch || {}), transcript_pages: verdictForDocument(printedPages) };
      onProgress({
        stage: 'chunking',
        message: printedPages.claimed
          ? `Transcript: cites use the reporter's printed page (${printedPages.reason})`
          : `Transcript: cites use the PDF page index — the printed page could not be read with confidence (${printedPages.reason})`,
      });
    }
    // Which route read the scanned pages and what it cost rides to the row
    // beside any other patch (an email's attachments), for a person to see.
    if (ocrRoute) metadataPatch = { ...(metadataPatch || {}), ocr_route: ocrRoute };
    if (outcome) metadataPatch = { ...(metadataPatch || {}), ingest_outcome: outcome };
    if (pendingOcr) {
      return await embedAndInsert(supabase, {
...
async function embedAndInsert(supabase, { documentId, matterspace_id, passages, openaiApiKey, onProgress = () => {}, seal = { sealed: false }, priorMetadata = null, pendingOcr = null, metadataPatch = null }) {
  ...
  // Westlaw star pages (2026-09-23): an opinion downloaded from Westlaw, in
  // any format, cites the reporter page each passage is on ("*561" in the
  // text) instead of "p. 1" or the PDF sheet. Here because this is the one
  // place every format's final passages pass through, in reading order. A
  // page another detector measured (a transcript's) is never overwritten.
  const westlaw = westlawStarPages(passages);
  let westlawCase = null;
  if (westlaw.claimed) {
    // The Bluebook cite for each passage (lib/bluebook.mjs), built from the
    // Westlaw header and the file name Westlaw gave the download.
    const { data: docRow } = await supabase.from('documents').select('source_filename').eq('id', documentId).maybeSingle();
    const { info, cites } = bluebookCitesFor(passages, westlaw, docRow?.source_filename ?? null);
    westlawCase = westlawCaseSummary(info);
    passages.forEach((p, i) => {
      const meta = westlawPageMeta(westlaw.pages[i], p.page_start);
      if (!meta || p.metadata?.printed_page != null) return;
      p.metadata = { ...(p.metadata || {}), ...meta, ...(cites[i] ? { bluebook_cite: cites[i] } : {}) };
    });
    onProgress({ stage: 'chunking', message: `Westlaw: cites use the reporter's page (${westlaw.reason})` });
  }
  if (westlaw.claimed || westlaw.markers || /restart|stray/.test(westlaw.reason)) {
    metadataPatch = {
      ...(metadataPatch || {}),
      westlaw_pages: { claimed: westlaw.claimed, reason: westlaw.reason, first: westlaw.first, last: westlaw.last },
      ...(westlawCase ? { westlaw_case: westlawCase } : {}),
    };
  }
  ...
  let embedded = 0;
  // Vectors owed: the route is configured but nothing is listening (a parked
  // SageMaker endpoint). Set only from the typed EmbedRouteUnavailableError.
  let embeddingPending = null;
  if (route) {
    try {
      ...
      if (!isEmbedRouteUnavailable(err)) throw err;
      // The parked endpoint (docs/SEALED_EMBEDDINGS_SETUP.md, "the switch you
      // control"). Not a failure to retry — the answer will not change for as
      // long as the endpoint is off — and never a reason to fall back to
      // another provider. Finish the way the no-route branch finishes: text
      // indexed locally and searchable by words, vectors null, nothing sent
      // anywhere else — and record on the row that the vectors are owed, so
      // the Vault, check_ingest_status and scripts/reembed-matter.mjs all
      // know. Batches that did embed before the endpoint went away keep their
      // vectors: they are real, and the backfill's `where embedding is null`
      // finds the rest.
      const owed = passages.filter((p) => !p.embedding).length;
      embeddingPending = {
        route: route.id,
        model: route.model,
        passages: owed,
      ...
      onProgress({
        stage: 'embedding',
        message: `${err.message} Indexing ${passages.length} passages for text search; ${owed} vector(s) owed — nothing sent elsewhere.`,
      });
    }
  } else {
    onProgress({
  ...
  for (let i = 0; i < passages.length; i += INSERT_CHUNK) {
    const batch = passages.slice(i, i + INSERT_CHUNK).map((p) => ({
      ...
      // A chunker may record what a passage IS in its own vocabulary (a
      // screenplay's scene heading) beside the constrained passage_type.
      // Always present: a bulk insert sends NULL, not the column default, for
      // a key that some rows in the batch carry and others do not — mixing
      // inferred-line transcript pages with prose pages violated the NOT NULL
      // constraint and lost six re-indexed depositions on 2026-09-10.
  ...
  // The record of which route OCR'd the pages (ocr_route) is this run's or
  // nobody's: a re-run that needed no OCR, or whose OCR failed, must not keep
  // a previous run's "read by Anthropic vision, $0.31" beside its passages.
  // The same goes for vectors owed (embedding_pending): a re-run that embeds
  // drops the record; a run that could not embed writes it fresh.
  // The printed-page verdict is this run's or nobody's for the same reason: a
  // re-run of a document that is no longer read as a transcript must not keep
  // yesterday's "cites the reporter's printed page" beside today's passages.
  const hadReason = prior?.metadata &&
    ('text_status' in prior.metadata || 'text_status_at' in prior.metadata || 'ocr_pending' in prior.metadata ||
      'ocr_route' in prior.metadata || 'embedding_pending' in prior.metadata || 'transcript_pages' in prior.metadata);
  if (hadReason || pendingOcr || metadataPatch || embeddingPending) {
    const { text_status: _ts, text_status_at: _at, ocr_pending: _op, ocr_route: _or, embedding_pending: _ep, transcript_pages: _tp, ...rest } = prior?.metadata || {};
    readyUpdate.metadata = {
      ...rest,
      ...(metadataPatch || {}),
      ...(pendingOcr ? { ocr_pending: pendingOcr } : {}),
      ...(embeddingPending ? { embedding_pending: embeddingPending } : {}),
    };
  }
  await supabase
    .from('documents')
  ...
  onProgress({
    stage: 'ready',
    message: (embeddingPending
      ? `${passages.length} passages indexed for text search — ${describeEmbeddingPending(embeddingPending)?.label || 'vectors owed'}; nothing sent elsewhere`
      : route
  ...
  return {
    passageCount: passages.length, embedded: Boolean(route) && !embeddingPending, embeddingModel: route?.model ?? null,
...
async function markStoredWithoutText(supabase, documentId, textStatus, { pageCount = null, message, onProgress = () => {}, metadata = {} } = {}) {
  ...
  // Nothing was read this run, so no route "read the pages" — a previous
  // run's ocr_route must not outlive it either.
  if (!('ocr_route' in metadata)) delete merged.ocr_route;
  // No passages, so no citations — a previous run's printed-page verdict is
  // stale too, and a verdict beside no text would read as a live claim.
  if (!('transcript_pages' in metadata)) delete merged.transcript_pages;
  // No passages, so no vectors owed — a previous run's embedding_pending is
  // stale too.
  if (!('embedding_pending' in metadata)) delete merged.embedding_pending;
...
export async function pagesNeedingOcr(fileBuf, pages) {
  ...
  // A page whose text layer was glyph codes has words on it, image or not.
  const unreadable = new Set(pages.filter((p) => p.unreadable).map((p) => p.pageNumber));
  const scanLike = await scanLikePages(fileBuf, short);
  if (scanLike === null) return short;
  return short.filter((n) => scanLike.has(n) || unreadable.has(n));
}
...
//
// `ocr(buf)` is the normalized runner: it may resolve to a bare page array
// (a hook) or to { pages, route } (a provider); `route` is carried out so the
// caller can record which route read the pages.
async function ocrPdfPages(fileBuf, pages, targets, ocr, onProgress = () => {}) {
  const wanted = new Set(targets);
  const ocrByPage = new Map();
  let route = null;
  const call = async (buf) => {
    const out = await ocr(buf);
    if (Array.isArray(out)) return out;
    route = out?.route || null;
    return out?.pages || [];
  };
  if (targets.length === pages.length) {
    const out = await call(fileBuf);
  ...
  return { ...mergeOcrPages(pages, ocrByPage, targets), route };
}

// The OCR option in either of its shapes (see processDocument), as one
// object: plan(tier) → { routes, reason }, run(buf, { tier, onProgress }) →
// { pages, route }. A bare hook has always been an unsealed provider (Gemini
// wired by the caller), so it serves Tier A and is refused for a sealed tier
// exactly as before Phase 4.
function normalizeOcrHook(ocr) {
  if (!ocr) return null;
  if (typeof ocr === 'object' && typeof ocr.run === 'function' && typeof ocr.plan === 'function') return ocr;
  if (typeof ocr === 'function') {
    return {
      plan: (tier) => (tier === 'B' || tier === 'C'
        ? { routes: [], reason: `No sealed OCR route is configured for Tier ${tier} where this ran.` }
        : { routes: [{ id: 'hook' }], reason: null }),
      run: async (buf) => ({ pages: (await ocr(buf)) || [], route: null }),
    };
  }
  return null;
}

// The record of what one run did with a PDF's pages, written to
// documents.metadata.ingest_outcome on every PDF outcome (indexed, image-only,
// awaiting OCR). Until 2026-09-18 nothing said whether a document's text came
// from its text layer or from OCR, so a scan indexed as its filing stamp
// looked exactly like a typed brief — to the monitor, to check_ingest_status
// and to anyone reading the row. The row now answers it:
//   ocr         not_needed | read | failed | held | not_configured
//   text_source text_layer | ocr | mixed | none — where the indexed words came from
//   pdf_pages   the PDF's own page count
//   ocr_pages   pages routed to OCR, as ranges ("1-2,7")
//   ocr_no_text pages OCR was sent and read nothing on (kept their own text)
// `pages` is the text-layer page list, before any OCR merge.
export function ingestOutcome(pages, ocrTargets, ocr, { noText = [] } = {}) {
  const targets = new Set(ocrTargets || []);
  const empty = new Set(noText || []);
  const typed = (pages || []).some((p) => !targets.has(p.pageNumber) && !isStampOnlyText(p.text) && (p.text || '').trim().length > 0);
  const read = ocr === 'read' && [...targets].some((n) => !empty.has(n));
  return {
    at: new Date().toISOString(),
    pdf_pages: (pages || []).length,
    ocr,
    text_source: read ? (typed ? 'mixed' : 'ocr') : (typed ? 'text_layer' : 'none'),
    ocr_pages: compactPages(ocrTargets) || null,
    ocr_no_text: compactPages(noText) || null,
  };
}

// [1,2,3,5,7,8] → "1-3,5,7-8"; empty → ''.
export function compactPages(nums) {
...
export async function extractPages(fileBuf, ext) {
  ...
  if (lower === '.rtf') {
    // Rich Text (Westlaw / Lexis downloads, older Word exports): prose inside
    // markup. Read as plain text it indexed the markup and failed embedding
    // 1,782 times on one Fleming opinion (found 2026-09-07).
    const { rtfToText } = await import('./rtf-text.mjs');
    return [{ pageNumber: 1, text: rtfToText(fileBuf) }];
  }
  if (lower === '.doc') {
    // Word 97–2003 binary (a .doc that is really something else was
    // re-routed by content before it got here).
    const { extractDocPages } = await import('./doc-extract.mjs');
    return extractDocPages(fileBuf);
  }
  if (lower === '.html' || lower === '.htm') return extractHtmlPages(fileBuf);
  // Plain text: one big "page", unless the file paginates itself — see
  // paginatePlainText. The chunker splits the rest into passages.
  const buf = fileBuf instanceof Uint8Array ? Buffer.from(fileBuf) : fileBuf;
  return paginatePlainText(buf.toString('utf8'));
}

// A transcript printed to a text file ("the TXT FILE copy" every reporter
// ships beside the PDFs) is paginated, and until 2026-09-21 none of that
// survived: the whole file arrived as one page, the chunker's size guard bailed
// on the blob, and every passage cited "p. 1". Two shapes, both unambiguous:
//
//   * FORM FEEDS. \f between pages is what a form feed means, and it is what
//     ASCII transcript printouts use. Splitting on it is not a heuristic.
//   * PAGE MARKER LINES. Older reporters number each page block instead —
//     "00015" or "Page 15" alone on a line. Ambiguous on its own (a numbered
//     list looks identical), so this shape is taken only when the markers run
//     +1 at least five times AND the file reads as testimony (Q./A. markers).
//
// Everything else stays exactly as it was: one page.
export function paginatePlainText(text) {
  const asPages = (parts) => {
    const kept = parts.map((t) => t.replace(/^\r?\n/, '')).filter((t) => t.trim().length > 0);
    return kept.length ? kept.map((t, i) => ({ pageNumber: i + 1, text: t })) : [{ pageNumber: 1, text }];
  };

  if (text.includes('\f')) {
    const parts = text.split('\f');
    if (parts.length > 1) return asPages(parts);
  }

  // A page marker is NOT just "a number on its own line" — that is also what
  // a blank numbered line in the reporter's line-number column looks like
  // (" 2"). A marker is zero-padded ("00015"), labelled ("Page 15"), or set
  // well away from the left margin, which the column never is.
  const lines = text.split('\n');
  const marks = [];
  for (let i = 0; i < lines.length; i++) {
    const raw = lines[i].replace(/\r$/, '');
    const m = /^\s*page\s+(\d{1,4})\s*$/i.exec(raw)
      || /^\s*0{2,}(\d{1,4})\s*$/.exec(raw)
      || /^ {8,}(\d{1,4})\s*$/.exec(raw);
    if (m) marks.push({ line: i, value: parseInt(m[1], 10) });
  }
  const qa = (text.match(/^[ \t]*(?:\d{1,2}[ \t]{1,6})?[QA]\.[ \t]/gm) || []).length;
  // A run of markers stepping by exactly +1. Anything less is a numbered list.
  let best = null;
  let run = null;
  for (const mark of marks) {
    if (run && mark.value === run.last + 1) { run.last = mark.value; run.marks.push(mark); }
    else run = { first: mark.value, last: mark.value, marks: [mark] };
    if (!best || run.marks.length > best.marks.length) best = run;
  }
  if (!best || best.marks.length < 5 || qa < 2) return [{ pageNumber: 1, text }];

  const parts = [];
  const starts = best.marks.map((m) => m.line);
  if (starts[0] > 0) parts.push(lines.slice(0, starts[0]).join('\n'));
  for (let i = 0; i < starts.length; i++) {
    const end = i + 1 < starts.length ? starts[i + 1] : lines.length;
    parts.push(lines.slice(starts[i], end).join('\n'));
  }
  return asPages(parts);
}

// A saved web page: tags, scripts and styles are markup, not evidence, so
// index the words a reader sees. And a page a court site serves INSTEAD of
// the document — CM/ECF's Case Search / login screen — is refused with the
// reason: eight Third Circuit "filings" in DeCamara › Appeal were that screen
// (2026-08-22 audit), indexed as four passages of HTML each.
function extractHtmlPages(fileBuf) {
  const html = (fileBuf instanceof Uint8Array ? Buffer.from(fileBuf) : fileBuf).toString('utf8');
  const title = ((html.match(/<title[^>]*>([\s\S]*?)<\/title>/i) || [])[1] || '').replace(/\s+/g, ' ').trim();
  const text = htmlToText(html);
  if (looksLikeCourtWebPage(text, title)) {
    throw new Error(`This is a court website page${title ? ` ("${title}")` : ''}, not the filing — CM/ECF saves its search and login screens as HTML when the document itself needs a PACER login. Download the filing from PACER as a PDF and upload that.`);
  }
...
export function htmlToText(html) {
  return String(html)
    ...
    .replace(/<!--[\s\S]*?-->/g, ' ')
    .replace(/<br\s*\/?>|<\/(?:p|div|li|tr|h[1-6]|blockquote|pre|table|section|article|header|footer|title)>/gi, '\n')
    .replace(/<[^>]+>/g, ' ')
...
// A short page carrying PACER's chrome and a search/login vocabulary is the
// court's screen, not a filing. A docket sheet or opinion printed to HTML
// carries the "PACER Service Center" receipt too, but runs far longer.
export function looksLikeCourtWebPage(text, title = '') {
  const t = `${title}\n${text}`;
  if (t.length > 6000) return false;
  return /PACER Service Center/i.test(t) && /(Case Search|Login|Logout|CM\/ECF)/i.test(t);
}

// pdf-parse's structural rejections. The file itself is often fine (see
// extractPdfPages); shared with the inline ingest paths so a serverless run
// hands such a file to the worker instead of failing it.
export function isPdfStructureError(err) {
  const m = String(err && err.message ? err.message : err || '').toLowerCase();
  return m.includes('bad xref') || m.includes('command token too long') || m.includes('invalid pdf structure') ||
    m.includes('stream must have data') || m.includes('illegal character');
}

// Rewrite a PDF through pdf-lib — a tolerant parser that emits a clean
// cross-reference table. Text and images survive; only the structure changes.
async function rewritePdf(buffer) {
  const { PDFDocument } = await import('pdf-lib');
...
async function extractDocxPages(buf) {
  ...
  try {
    ...
    // A .docx is a zip; "can't find end of central directory" means the bytes
    // stop short (an upload cut off) or were never Word's. Say so in words —
    // the raw zip error left a declaration "unclassified" on 2026-09-07.
    throw new Error(`The Word file could not be opened (${firstLine(err.message || String(err))}) — it was cut off during upload or is not a real .docx. Open it in Word, Save As .docx, and upload it again.`);
  }
...
// A page whose text layer is glyph codes (lib/text-quality.mjs) is returned
// with its text emptied and `unreadable: true`: from here on it is exactly a
// scanned page — routed to the worker, OCR'd whether or not it carries an
// image (a Type 3 text page has none), and recorded as awaiting OCR when OCR
// cannot run — instead of being indexed as noise under a "ready" status.
// Cornell LII's Rule 56, printed from a browser, was all such pages
// (DeCamara, 2026-09-24).
async function extractPdfPages(buf) {
  const pages = await extractPdfPagesRaw(buf);
  // Page numbers a PDF draws in Watermark annotations (Acrobat's Header &
  // Footer) — invisible to pdf.js. Carried beside the text for the printed-
  // page detector only, never indexed (lib/pdf-overlay-text.mjs).
  let overlay = null;
  try {
    const { pdfOverlayText } = await import('./pdf-overlay-text.mjs');
    overlay = await pdfOverlayText(buf instanceof Uint8Array ? Buffer.from(buf) : buf);
  } catch { overlay = null; }
  return pages.map((pg) => {
    const withOverlay = overlay?.get(pg.pageNumber) ? { ...pg, overlayText: overlay.get(pg.pageNumber) } : pg;
    return isUnreadableText(withOverlay.text) ? { ...withOverlay, text: '', unreadable: true } : withOverlay;
  });
}

async function extractPdfPagesRaw(buf) {
  ...
  // The PDF's own page count, from its page tree (pdf.js numPages). page_count
  // and the OCR routing both follow it, never the number of text chunks the
  // extractor happened to return — see fitToPageCount.
  let numPages = null;
  const tryParse = async (bytes) => {
    pages.length = 0;
    try { numPages = (await pdfParse(bytes, options))?.numpages ?? null; return true; } catch { pages.length = 0; return false; }
  };
  try {
    numPages = (await pdfParse(buffer, options))?.numpages ?? null;
  } catch (err) {
    ...
    // read with it before calling the file corrupt.
    //
    // It is not even deterministic (2026-09-07): on a set of ReportLab cover
    // sheets the FIRST pdfParse call in a fresh process fails "bad XRef
    // entry" and the second reads the same bytes fine — and a cold
    // serverless function is always on its first call, which is how ten
    // cover pages filed through the MCP failed in one afternoon. So: read
    // again, then modern pdfjs, then the bytes rewritten through pdf-lib,
    // and only then call the file corrupt, with pdf-parse's message as before.
    if (!(await tryParse(buffer))) {
      const modern = await extractPdfPagesModern(buffer).catch(() => null);
      if (modern) return modern;
      const rewritten = await rewritePdf(buffer).catch(() => null);
      if (!rewritten || !(await tryParse(rewritten))) throw err;
    }
  }
  ...
  if (pages.length === 0) {
    ...
    numPages = result?.numpages ?? numPages;
    const raw = result.text || '';
    const parts = raw.includes('\f') ? raw.split('\f') : [raw];
    const split = parts.map((text, i) => ({ pageNumber: i + 1, text }));
    // A split that does not match the page tree put every page's text on
    // "page 1" — how two scanned 2-page orders were stored as 1 page on
    // 2026-05-25. Read page by page with modern pdfjs before settling for it.
    if (numPages && split.length !== numPages) {
      const modern = await extractPdfPagesModern(buffer).catch(() => null);
      if (modern && modern.length === numPages) return modern;
    }
    return fitToPageCount(split, numPages);
  }
  return fitToPageCount(pages.map((text, i) => ({ pageNumber: i + 1, text })), numPages);
}

// Make the extracted page list agree with the PDF's page tree, so that
// page_count is the PDF's and every page is present to be routed to OCR. A
// page the extractor never returned is added as empty (a scanned page with no
// text layer looks exactly like that). Text past the last real page — a
// trailing form feed, an extractor quirk — is folded into the last page, never
// dropped. With no count to go by, the list is returned as it came.
export function fitToPageCount(pages, numPages) {
  const n = Number(numPages);
  if (!Number.isInteger(n) || n < 1 || pages.length === n) return pages;
  if (pages.length < n) {
    return [...pages, ...Array.from({ length: n - pages.length }, (_, i) => ({ pageNumber: pages.length + i + 1, text: '' }))];
  }
  const kept = pages.slice(0, n).map((p) => ({ ...p }));
  const extra = pages.slice(n).map((p) => p.text || '').filter((t) => t.trim()).join('\n');
  if (extra) kept[n - 1].text = [kept[n - 1].text, extra].filter(Boolean).join('\n');
  return kept;
}
...
// -----------------------------------------------------------------------------
// First pass: every page in the shape it came in. Then, if the document is
// a transcript on the whole (half its pages or more), read again the prose
// pages that sit BETWEEN transcript pages with the relaxed test — a long
// answer with no Q./A. on the page is still testimony, and the reader
// wants its lines. Front matter and the word index stay prose.
function parseAllTranscriptPages(pages) {
  const parsed = pages.map(({ text }) => parseTranscriptPage(text));
  const isT = parsed.map((p) => !!p);
  const first = isT.indexOf(true);
  const last = isT.lastIndexOf(true);
  if (first >= 0 && isT.filter(Boolean).length * 2 >= pages.length) {
    for (let i = first + 1; i < last; i++) {
      if (!parsed[i]) parsed[i] = parseTranscriptPage(pages[i].text, { relaxed: true });
    }
  }
  return parsed;
}
...
/**
 * The printed-page verdict for a document, without chunking it.
 *
 * The audit script (`scripts/audit-transcript-pages.mjs`) and the offline
 * harness use this; ingestion gets the same verdict out of chunkPages, which
 * needs the parse anyway.
 */
export function analyzeTranscriptPages(pages) {
  return detectPrintedPages(pages, parseAllTranscriptPages(pages));
}

// What a passage records about its own page number.
//
//   page_source: 'printed'    — printed_page is the reporter's own number.
//   page_source: 'pdf_index'  — the detector looked and would not claim one;
//                               page_start is the PDF's index, and
//                               printed_page_confidence says how close it got.
//   neither key               — indexed before this detector existed (or not a
//                               transcript). The standing "page may be the PDF
//                               index" note is the only honest thing to say.
//
// A citation builder should prefer `metadata.printed_page`, fall back to
// `page_start` with the caveat when `page_source === 'pdf_index'`, and keep
// today's unconditional standing note when neither key is present.
function pageMetaFor(verdict, pdfPage, chunkPage) {
  if (!verdict?.is_transcript) return null;
  if (verdict.method === METHOD.CONDENSED) {
    // The panel already carries the reporter's page; what was being thrown
    // away is the PDF page the panel was printed on, which is what the Reader
    // needs to open the sheet. Record both.
    return { printed_page: chunkPage, printed_page_end: chunkPage, pdf_page: pdfPage,
      page_source: 'printed', printed_page_confidence: verdict.confidence, printed_page_method: verdict.method };
  }
  const printed = printedPageFor(verdict, pdfPage);
  if (printed == null) {
    // printed_page_confidence is about THIS page, not the document. A page
    // that fell outside every run in an otherwise-confident document (a slip
    // sheet, the word index, the errata) knows nothing at all — writing the
    // document's "high" here would put the word "high" next to a caveat.
    return {
      page_source: 'pdf_index',
      printed_page_confidence: verdict.claimed ? 'none' : verdict.confidence,
    };
  }
  return { printed_page: printed, printed_page_end: printed, pdf_page: pdfPage,
    page_source: 'printed', printed_page_confidence: verdict.confidence, printed_page_method: verdict.method };
}
...
export function chunkPages(pages, opts = {}) {
  ...
  const parsed = parseAllTranscriptPages(pages);

  // Which number the reporter PRINTED on each page — never assumed to be the
  // PDF's page index. `opts.report` is an out-parameter rather than a second
  // return value because three callers consume chunkPages' array directly.
  const verdict = detectPrintedPages(pages, parsed);
  if (opts.report && typeof opts.report === 'object') opts.report.printedPages = verdict;

  pages.forEach(({ pageNumber, text }, idx) => {
    const transcript = parsed[idx];
    if (transcript) {
      for (const c of transcript.chunks) {
        ...
        // A condensed sheet holds several transcript pages; the chunk's own
        // page is the one a cite names.
        const page = c.page ?? pageNumber;
        const pageMeta = pageMetaFor(verdict, pageNumber, page);
        const meta = { ...(c.metadata || {}), ...(pageMeta || {}) };
        passages.push({
          sequence_number: seq++,
          page_start: page,
          page_end: page,
          ...
          // Inferred line numbers (reporter PDFs with no numbers in the text
          // layer) are flagged on the passage so a cite can be checked, and
          // so is where this passage's page number came from.
      ...
      // A prose page inside a transcript (the caption, the appearances page,
      // the word index) is still cited as part of the transcript, so it gets
      // the same page-source record as the testimony around it.
      const pageMeta = pageMetaFor(verdict, pageNumber, pageNumber);
...
export async function chunkFountain(text) {
  ...
  // passages.passage_type is check-constrained to the deposition / book
  // vocabulary (migrations 002 + 007); the screenplay vocabulary above is
  // mapped onto it here and kept, by name, in metadata.element. Every
  // .fountain failed that constraint from June to September 2026 — parked
  // "Unclassified failure" until the nightly suite named it.
  const TYPE_FOR = {
    title_page: 'section_heading', scene_heading: 'section_heading', section_heading: 'section_heading',
    action: 'monologue', character_dialogue: 'monologue', transition: 'monologue', centered: 'monologue',
    synopsis: 'summary', note: 'footnote',
  };
  return passages.map((p) => ({
    ...p,
    passage_type: TYPE_FOR[p.passage_type] || 'monologue',
    metadata: { ...(p.metadata || {}), element: p.passage_type },
  }));
}


// "35", "00035", "Page 35", "Page 35 of 240" — the number the reporter printed
// above or below the text, on a line of its own. Nothing a witness says looks
// like this, and it is not a line of testimony, so it is not numbered as one.
const PRINTED_PAGE_MARKER_RE = /^(?:page\s+)?0*\d{1,4}(?:\s+of\s+0*\d{1,4})?$/i;
function stripPrintedPageMarkers(lines) {
  let from = 0;
  let to = lines.length;
  while (from < to && PRINTED_PAGE_MARKER_RE.test(lines[from])) from += 1;
  while (to > from && PRINTED_PAGE_MARKER_RE.test(lines[to - 1])) to -= 1;
  return lines.slice(from, to);
}

// Reporter transcripts arrive in four text-layer shapes, and a page must be
// read in whichever one it came in:
//   numbered   — "12   Q.  Where were you…" (the number in the text layer);
//   bare       — Veritext: no numbers at all, middle-dot spacing; lines are
//                numbered by position and flagged metadata.line_numbers =
//                'inferred';
//   condensed  — four transcript pages per PDF page ("Page 34" markers, the
//                number on its own line, the text on the next); every chunk
//                carries the transcript page it came from;
//   relaxed    — an interior page of a document that is otherwise a
//                transcript, with fewer than two Q/A markers on it (a long
//                answer); chunkPages asks for this on a second pass.
// The witness is read from the call: "NAME, having been first duly sworn"
// on one line; a bare name line (letter-spaced or not — "D E S M O N D
// B L A K E," is how New York reporters set it) up to three lines above the
// oath; or a page header "(JOON PARK, M.D. - CONFIDENTIAL TESTIMONY)".
function parseTranscriptPage(pageText, { relaxed = false } = {}) {
  // Size guard: transcript pages are ~1.5-3K chars each; condensed pages run
  // to ~5K. Anything well above that is a concatenated blob — let
  // paragraphChunks handle it as prose.
  const condensedMarks = (pageText.match(/^Page \d+\s*$/gm) || []).length;
  if (pageText.length > (condensedMarks >= 2 ? 12000 : 8000)) return null;

  // Veritext-style text layers space with middle dots ("· · ·Q.· ·Where…")
  // and non-breaking spaces; read them as spaces so "Q." sits at a line start.
  pageText = pageText.replace(/[· ]/g, ' ');

  const headerRE = /\b(DIRECT|CROSS|REDIRECT|RECROSS|VOIR DIRE)\s+EXAMINATION\b/i;
  const witnessCallRE = /\b([A-Z][A-Z'\-\. ]+?),\s+(?:having (?:first )?been|was|called as a witness)\b/;
  const nameLineRE = /^([A-Z][A-Z'\-\.]*(?:\s+[A-Z][A-Z'\-\.]*){1,4})(?:,\s*(?:M\.D\.|PH\.D\.|D\.O\.|R\.N\.|ESQ\.|JR\.|SR\.|III))?,?\s*(?:the\s+)?$/;
  const callLineRE = /\b(?:having (?:first )?been(?: first)? duly sworn|duly sworn|testified as follows|examined and testified|was sworn|called as a witness|the witness herein|witness herein)\b/i;
  const headerNameRE = /^\(([A-Z][A-Z'\-\. ]+?)(?:,\s*(?:M\.D\.|PH\.D\.|D\.O\.|R\.N\.|ESQ\.|JR\.|SR\.))?\s*(?:-\s*[A-Z ]+)?\)$/;

  // Letter-spaced names ("D E S M O N D   B L A K E,") collapse to words.
  const collapseLetters = (s) => {
    let t = s.trim();
    if (!/^(?:[A-Z]\s)+[A-Z]/.test(t)) return s;
    // "J O H A N N E   S A I N T-F L E U R, the" → the trailing ", the" (the
    // Witness herein…) and commas are not part of the name.
    t = t.replace(/,?\s*the\s*$/i, '').replace(/,\s*$/, '');
    return t.split(/\s{2,}/).map((w) => w.replace(/\s+/g, '')).join(' ');
  };

  // ---- 1. Read the page into numbered lines ---------------------------------
  let lines = [];
  let inferred = false;
  let condensed = false;
  if (condensedMarks >= 2) {
    // Condensed: "Page 34" then alternating number / text lines.
    condensed = true;
    let page = null;
    let pending = null;
    for (const raw of pageText.split('\n')) {
      const t = raw.trim();
      if (!t) continue;
      const pm = /^Page (\d+)\s*$/.exec(t);
      if (pm) { page = parseInt(pm[1], 10); pending = null; continue; }
      if (/^\d+ \(Pages \d+ to \d+\)$/.test(t)) continue;         // the sheet header
      const nm = /^(\d{1,2})$/.exec(t);
      if (nm) {
        // A number with no text before the next number was a blank line.
        if (pending !== null) lines.push({ lineNum: pending, text: '', page });
        pending = parseInt(nm[1], 10); continue;
      }
      const inline = /^(\d{1,2})\s{1,6}(.*)$/.exec(t);
      if (inline && page !== null) { lines.push({ lineNum: parseInt(inline[1], 10), text: inline[2].trim(), page }); pending = null; continue; }
      if (pending !== null) { lines.push({ lineNum: pending, text: t, page }); pending = null; }
      else if (lines.length && page !== null) lines[lines.length - 1].text += '\n' + t;
      else lines.push({ lineNum: 0, text: t, page });                 // a header line before line 1
    }
    lines = lines.filter((l) => l.lineNum >= 0 && l.lineNum <= 40);
    if (lines.length < 8) return null;
  } else {
    // One numbered line per match, and NEVER across a newline: the gap after
    // the number is spaces/tabs only, and a numbered line may carry no text.
    // (Until 2026-09-10 the gap was `\s{1,6}`, which matched the newline on a
    // blank numbered line, so the next line's text was read as the blank
    // line's and the Q./A. test never fired — a whole deposition fell to prose.)
    //
    // And nothing ABOVE the line-number column is a line. The reporter prints
    // the page number in the running header, alone on its line — "35" — which
    // is shaped exactly like a line number. Read as one it becomes line 35 of
    // a 25-line page, and under the pre-2026-09-10 gap it also swallowed the
    // newline and took the real line 1's text with it: the cite "35:35-7" on
    // a passage whose text began "1  A. It did…" (Gary Pruett, PDF p. 35 —
    // still in the demo record today, indexed before that fix). The gap no
    // longer crosses a newline, but the header is still read as a blank line
    // 35 on every page of every transcript numbered under 41. The column's own
    // start says where lines begin. Same cause as the inferred off-by-one
    // recorded in PR #205 §8, fixed the same way in the branch below.
    const { raw, start: columnStart } = lineColumnBounds(pageText);
    const lineRE = /^[ \t]{0,6}(\d{1,2})(?:[ \t]{1,6}(.*))?$/;
    for (let i = 0; i < raw.length; i++) {
      const m = lineRE.exec(raw[i]);
      if (!m) continue;
      let n = parseInt(m[1], 10);
      let text = m[2] ?? '';
      if (columnStart >= 0 && i < columnStart) {
        // Above the column: this is the printed page number, not a line. A
        // text layer that sets the header and the first line on one baseline
        // welds them ("35 1  A. It did…") and that line IS testimony — read it
        // as the line it carries, and drop the page number.
        const welded = /^(\d{1,2})[ \t]{1,6}(.*)$/.exec(text.trim());
        if (!welded) continue;
        n = parseInt(welded[1], 10);
        text = welded[2];
      }
      if (n < 1 || n > 40) continue;
      lines.push({ lineNum: n, text: text.trim() });
    }
    if (lines.length < 8) {
      // No numbers in the text layer (Veritext): number the text lines by
      // position and say so on every chunk. A page with blank numbered lines
      // in the original will be off by those lines; the flag is what lets a
      // citation be checked before it is used.
      const nonEmpty = pageText.split('\n').map((l) => l.trim()).filter((l) => l.length > 0);
      const markers = nonEmpty.filter((l) =>
        /^(Q\.|A\.)\s/.test(l) || /^BY\s+(MR\.|MRS\.|MS\.|DR\.)/.test(l) || headerRE.test(l) || callLineRE.test(l)).length;
      if (nonEmpty.length < 15 || nonEmpty.length > 32 || markers < (relaxed ? 0 : 2)) return null;
      // The page number the reporter printed is not testimony. With no numbers
      // in the text layer it is simply the first thing extracted — a bare "35"
      // — and counting it as text line 1 put every inferred cite on this page
      // one line low (recorded, not fixed, in PR #205 §8). The guard above
      // still counts it, so no page changes its reading; only the numbering.
      lines = stripPrintedPageMarkers(nonEmpty).map((text, i) => ({ lineNum: i + 1, text }));
      inferred = true;
    }
  }

  // ---- 2. Is it a transcript at all? -----------------------------------------
  // Numbered-line detection alone is too eager — case-law PDFs carry plenty of
  // 1-2 digit prefixes (footnotes, star pages, list items). Require a real
  // transcript marker before committing; a relaxed pass (an interior page of a
  // document already known to be a transcript) is exempt.
  const looksLikeTranscript = relaxed || lines.some(({ text }) =>
    /^Q\.\s/.test(text) ||
    /^A\.\s/.test(text) ||
    /^BY\s+(MR\.|MRS\.|MS\.|DR\.)/.test(text) ||
    headerRE.test(text) ||
    callLineRE.test(text) ||
    witnessCallRE.test(text)
  );
  if (!looksLikeTranscript) return null;

  const chunkMeta = inferred ? { metadata: { line_numbers: 'inferred' } } : {};

  // ---- 3. Chunk into headings and Q/A pairs ----------------------------------
  const chunks = [];
  let cur = null;
  function flush() {
  ...
  let pageWitness = null; // from a "(NAME - CONFIDENTIAL TESTIMONY)" header
  // The oath runs over several lines ("called as a witness, having been first
  // duly / sworn by a Notary Public…, was examined and / testified as
  // follows:"); every line until "follows:" belongs to the call.
  let inOath = false;
  let oathLines = 0;

  for (let i = 0; i < lines.length; i++) {
    const { lineNum, text, page } = lines[i];
    const header = headerRE.exec(text);
    const witnessCall = witnessCallRE.exec(collapseLetters(text));
    let witnessName = witnessCall ? witnessCall[1].trim() : null;
    // Two-line call: this line is a bare name and one of the next lines takes
    // the oath; or the oath line looks back at a bare name above it.
    const upcoming = [lines[i + 1], lines[i + 2]].map((l) => l?.text || '').join(' ');
    const collapsed = collapseLetters(text);
    const nameBeforeCall = !witnessCall && nameLineRE.test(collapsed) && callLineRE.test(upcoming);
    if (nameBeforeCall) witnessName = nameLineRE.exec(collapsed)[1].trim();
    if (!witnessName && callLineRE.test(text)) {
      for (let j = i - 1; j >= 0 && j >= i - 3; j--) {
        const prev = collapseLetters(lines[j].text || '');
        if (!prev) continue;
        const nm = nameLineRE.exec(prev);
        if (nm) { witnessName = nm[1].trim(); break; }
      }
    }
    const headerName = headerNameRE.exec(text);
    if (headerName && !pageWitness) pageWitness = headerName[1].trim().replace(/,\s*$/, '');
    // "THEREUPON," / "WHEREUPON," is the reporter's cue before the call.
    let isCall = !!witnessCall || nameBeforeCall || callLineRE.test(text) || /^(THEREUPON|WHEREUPON),?$/i.test(text);
    const endsOath = /\b(?:as follows|follows)\s*:?\s*$/i.test(text);
    if (inOath) { isCall = true; oathLines++; }
    if (isCall && callLineRE.test(text) && !endsOath) { inOath = true; oathLines = 0; }
    if (endsOath || oathLines >= 4 || header) { inOath = false; oathLines = 0; }
    const isQ = /^Q\.\s/.test(text);
    const isA = /^A\.\s/.test(text);
    // "BY MR. QUAINTON:" on one line, or New York's "EXAMINATION BY" with the
    // examiner's bare "MR. QUAINTON:" on the next — neither is testimony.
    const isExamBy = /^BY\s+(MR\.|MRS\.|MS\.|DR\.)/.test(text) ||
      /^(?:CONTINUED\s+)?EXAMINATION\s+BY\b/i.test(text) ||
      /^(?:MR|MRS|MS|DR)\.\s+[A-Z][A-Z'\-\. ]+:$/.test(text);

    // A blank numbered line neither starts a chunk nor stretches one: the
    // cite "8:10-11" must span the words, not the reporter's white space.
    if (!text) continue;
    // The condensed sheet's page header is not testimony.
    if (headerName) { flush(); continue; }
    // A new transcript page inside a condensed sheet closes the open chunk.
    if (cur && page != null && cur.page != null && page !== cur.page) flush();

    if (header || isCall || isExamBy) {
      flush();
      chunks.push({
        line_start: lineNum, line_end: lineNum, text, page,
        passage_type: 'section_heading',
  ...
  // A witness named only in the page header goes on the first chunk, so
  // chunkPages carries it forward.
  if (pageWitness && chunks.length && !chunks.some((c) => c.witness_name)) chunks[0].witness_name = pageWitness;

  const merged = [];
  for (const c of chunks) {
    const last = merged[merged.length - 1];
    if (last && last.passage_type === c.passage_type && last.text.length < 300 && (last.page ?? null) === (c.page ?? null)) {
      last.line_end = c.line_end;
      last.text += '\n' + c.text;
      // Merged headings keep what any of them knew: the witness from the
      // call line, the examination type from the "DIRECT EXAMINATION" line.
      last.witness_name ??= c.witness_name;
      last.examination_type ??= c.examination_type;
    } else {
      merged.push(c);
    }
  }
  return { chunks: merged, condensed };
}
...
export async function embedBatch(
  ...
  for (;;) {
    ...
    // The route's own reading of "nothing is listening" — a SageMaker endpoint
    // that is switched off, still creating, or never provisioned answers 400
    // NO_SUCH_ENDPOINT. Neither a bad request nor an outage to retry through:
    // a typed, terminal answer callers hold on, rather than burn a job's
    // attempt budget against (lib/embed-routes.mjs, EmbedRouteUnavailableError).
