...
export async function processDocument(supabase, options) {
  ...
  if (lowerExt === '.epub') {
    ...
    if (pendingOcr) {
      return await embedAndInsert(supabase, {
        documentId, matterspace_id, passages, openaiApiKey, onProgress, seal, priorMetadata, pendingOcr,
      });
    }
  }
...
async function embedAndInsert(supabase, { documentId, matterspace_id, passages, openaiApiKey, onProgress = () => {}, seal = { sealed: false }, priorMetadata = null, pendingOcr = null }) {
  ...
  if (passages.length === 0) {
    ...
    if (pendingOcr) {
      return await markStoredWithoutText(supabase, documentId, TEXT_STATUS.OCR_PENDING, {
        onProgress, metadata: { ocr_pending: pendingOcr },
  ...
  if (hadReason || pendingOcr) {
    const { text_status: _ts, text_status_at: _at, ocr_pending: _op, ...rest } = prior?.metadata || {};
    readyUpdate.metadata = pendingOcr ? { ...rest, ocr_pending: pendingOcr } : rest;
  }
  await supabase
    .from('documents')
    .update(readyUpdate)
