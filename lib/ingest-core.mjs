...
export async function processDocument(supabase, options) {
  ...
  if (lowerExt === '.epub') {
    ...
    const pages = await extractPages(fileBuf, lowerExt);
...
// pdf-parse bundles its own pdfjs (older, no DOMMatrix init) so it works in
// Vercel's serverless runtime where pdfjs-dist 5.x crashes at module load.
// Page boundaries come back as form-feed (\f) characters in the joined text.
async function extractPdfPages(buf) {
  const { default: pdfParse } = await import('pdf-parse');
  const buffer = buf instanceof Uint8Array ? Buffer.from(buf) : buf;
  const result = await pdfParse(buffer);
  const raw = result.text || '';
  // Form-feeds delimit pages in pdf-parse output. Some PDFs don't emit any
  // form-feeds (single-page or stream-style); in that case treat the entire
  // text as page 1 so the chunker still sees something.
