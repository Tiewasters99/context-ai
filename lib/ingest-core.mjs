...
export async function processDocument(supabase, options) {
  ...
  if (lowerExt === '.epub') {
    ...
    const finishAsPortfolio = async (portfolio) => {
      ...
      await supabase
        ...
        .update({
          ...
          metadata: {
            ...(parentRow.metadata || {}),
            portfolio: summary,
            text_status: TEXT_STATUS.PORTFOLIO,
      ...
      return { passageCount: 0, portfolio: summary, textStatus: TEXT_STATUS.PORTFOLIO };
    };
    ...
    // Scanned, and no OCR hook: the honest outcome is a failure with a cause,
    // not the old "no passages extracted" (which triage read as a benign
    // photo) and not a silent store. The message names the fix; the triage
    // table maps it to ocr_needed.
