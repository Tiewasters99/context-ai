...
// 4 chars/token is a fair *average* for English prose, which is what the
// batching maths wants. It is a bad worst case, and the per-input hard cap is
// exactly where the worst case bites: OCR garble, tables, citation-dense legal
// text and long unbroken identifiers all tokenize nearer 2-3 chars/token, so
// 8000 estimated tokens of that is well over the model's real 8192 ceiling.
// Truncating at MAX_INPUT_TOKENS * CHARS_PER_TOKEN therefore re-used the very
// assumption that overflowed, and the request 400'd again — deterministically,
// on 593 documents. Cap on a pessimistic ratio instead; embedBatch still
// re-truncates on a 400, so this only has to be close, not perfect.
...
// JPEG and PNG streams, and it does so losslessly, so these route through the
// same `ocr` hook as scanned PDFs. Exported so every ingestion entry point
// wires the hook for these exactly as it does for '.pdf'.
export const OCRABLE_IMAGE_EXTENSIONS = ['.jpg', '.jpeg', '.png'];
...
export async function processDocument(supabase, options) {
  ...
  // must come out searchable like any scanned PDF. We wrap the bytes in a
  // one-page PDF (lossless: pdf-lib embeds the JPEG/PNG stream as-is) and
  // run the same injected `ocr` hook as scanned PDFs, so OCR behavior can't
  // drift between the two paths. Formats pdf-lib can't embed (.gif/.webp/
  // .svg/.bmp/.tiff), callers with no OCR hook, and images OCR finds no
  // legible text in (photos, logos) all keep store-and-display behavior.
  if (IMAGE_EXTENSIONS.includes(lowerExt)) {
    if (typeof ocr === 'function' && OCRABLE_IMAGE_EXTENSIONS.includes(lowerExt)) {
      ...
      if (ocrChars >= 40) {
        await supabase
          .from('documents')
          .update({ page_count: 1, processing_status: 'chunking' })
          .eq('id', documentId);
...
// Wrap a JPEG/PNG in a single-page PDF so the standard `ocr` hook (which
// takes PDF bytes) can read it. pdf-lib embeds the compressed image stream
// directly — no re-encode — so OCR sees exactly the uploaded pixels.
async function imageToPdf(fileBuf, ext) {
