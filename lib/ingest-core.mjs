...
export async function processDocument(supabase, options) {
  ...
  if (lowerExt === '.epub') {
    ...
    if (lowerExt === '.pdf') {
      const { detectPdfPortfolio, unpackPortfolio } = await import('./pdf-portfolio.mjs');
      let portfolio = null;
      try {
        portfolio = await detectPdfPortfolio(fileBuf);
      } catch (err) {
      ...
      if (portfolio) {
        ...
        await supabase
          ...
          .update({
            processing_status: 'ready',
            processing_error: null,
            page_count: portfolio.pageCount,
            ingested_at: new Date().toISOString(),
            metadata: { ...(parentRow.metadata || {}), portfolio: summary },
          })
          .eq('id', documentId);
        onProgress({
          stage: 'ready',
        ...
        return { passageCount: 0, portfolio: summary };
      }
    }
...
async function embedAndInsert(supabase, { documentId, matterspace_id, passages, openaiApiKey, onProgress = () => {}, seal = { sealed: false } }) {
  ...
  onProgress({
    ...
    message: route
      ...
      : `${passages.length} passages indexed for text search — nothing sent out`,
  });

  return { passageCount: passages.length, embedded: Boolean(route), embeddingModel: route?.model ?? null };
}
