...
export function choosePen({ tier, anthropicKey, fireworksKey, bedrockCreds = null, escalate = false }) {
  ...
  if (t === 'B') {
    ...
    if (bedrockCreds) {
      ...
      // Frontier Claude inside the seal: escalation is moot. A non-Anthropic
      // sealed pen keeps the recorded escalation path open (user-requested,
      // never silent).
      if (pen.route === 'messages' || !escalate) return { ...pen, creds: bedrockCreds, escalation: false };
