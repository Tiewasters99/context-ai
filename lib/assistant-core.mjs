...
const MATTER_SCOPED_TOOLS = new Set(['search', 'grep', 'list_matter_contents', 'get_outline', 'check_ingest_status']);
...
export async function runAssistantStream({ supabase, anthropicKey, openaiApiKey, messages, matterId, context, emit }) {
  ...
  try {
    ...
    const system = buildOrchestratorSystem({ matterId, today, ...(context || {}) });
    ...
    for (let i = 0; i < MAX_ITERATIONS; i++) {
      ...
      for (const tu of toolUses) {
        ...
        if (tu.name === 'relay_feedback') {
          ...
          if (!body) {
            results.push({ type: 'tool_result', tool_use_id: tu.id, content: 'Error: feedback body was empty.', is_error: true });
            continue;
          }
          const row = {
