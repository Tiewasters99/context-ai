...
import {
  matterTierWithClient, providerAllowed, isSealedTier,
...
import { SEALED_PEN_PREAMBLE, applyPenPreamble } from './pen-preambles.mjs';
...
//
// `preamble` (2026-09-20) is what this pen is told about where it is, before
// the feature's own system prompt (lib/pen-preambles.mjs). Only the SEALED
// pens declare one: it is a fact about working inside a sealed matter, and it
// is attached by the pen table rather than by a tier test at each call site,
// so "only the sealed pen gets it" holds for every surface — the Assistant,
// the meeting panel and every /api/llm feature — without any of them knowing
// the preamble exists. PENS.anthropic has none, which is also what makes a
// Tier-B ESCALATION (`{...PENS.anthropic, escalation:true}`, a turn taken
// deliberately outside the seal) byte-identical to a Tier-A turn.
export const PENS = {
  anthropic: {
    provider: 'anthropic',
  ...
  bedrockOpen: {
    ...
    // How much this pen can be given at once. Every other pen in the product
    // carries this figure (src/lib/llm/providers.ts, contextWindow); the
    // sealed pen was the only model in the codebase with no declared size, so
    // anything sizing a request for it had nothing to read.
    //
    // 256,000 is the catalog's own figure for the neighbouring Kimi K2.6
    // entry — src/lib/llm/providers.ts:46 (and :57 for the Fireworks mirror
    // of the same weights). It is NOT a verified number for K2.5 on Bedrock,
    // and it is not invented either: it is the nearest figure this repo
    // already states for this model family, and it is the SMALLER of the two
    // Kimi sizes on record (K3 is 1,000,000 at :45), so using it errs toward
    // sending less. Replace it with AWS's published figure when one exists.
...
/**
 * Tools whose EFFECT leaves the matter. Not offered inside a seal.
 *
 * The test is not "does it write" — set_matter_state writes, and it writes
 * inside the matter, under the user's own RLS, into an append-only ledger.
 * The test is whether the act ends up somewhere the seal does not reach:
 *
 *   relay_feedback  inserts into orchestrator_feedback AND mirrors the
 *                   feedback text into the Contextspaces ADMIN matter's
 *                   matter_state_events. A sentence spoken inside a sealed
 *                   matter lands in another matter's chronology, read by
 *                   people who are not on this one. No model sees it, so it
 *                   is not a seal breach — it is a breach of matter
 *                   isolation (feedback_strict_matter_isolation), which is
 *                   the contract the seal is supposed to make absolute.
 *
 *   move_document   proposes carrying a document OUT of this matter into
 *                   another one, which may be unsealed — and an unsealed
 *                   matter's next ingest embeds it at OpenAI and shows it to
 *                   connectors. The browser still asks the user to confirm,
 *                   but a sealed run should not be composing that proposal
 *                   in the first place.
 *
 * Deliberately NOT here: create_sub_matter (the child is inside this matter
 * and inherits the seal); open_document / open_matter (they navigate the
 * user's own browser and export nothing); list_matters (it reads other
 * matters' names IN to a pen that is already inside the seal, which is the
 * direction the seal does not govern); ingest_document and set_matter_state
 * (both stay in the matter).
 */
export const LEAVES_THE_MATTER = Object.freeze(new Set(['relay_feedback', 'move_document']));
 ...
 * `toolNames` (optional): an explicit list that NARROWS what this run may be
 * offered — MCP tools, the two navigation actions, the two confirm-gated
 * writes and the feedback relay alike. Omitted, the run is offered exactly
 * what it is offered today. Like a charter, it can only ever take away: a
 * name not already on the menu is ignored, and a sealed matter drops
 * LEAVES_THE_MATTER whether or not this list was supplied. It exists so a
 * surface that is not chat — the meeting route — can run this loop with a
 * narrower menu without writing a second loop.
 *
...
export async function runAssistantStream({
  ...
  // ── W3 the pause. Asked before the pen is chosen, again at the top of every
  // tool round, and again before every single tool call. `pauseDeployed` goes
  // false the first time the schema says migration 070 is not here, and the
  // later checks then cost nothing.
  let pauseDeployed = true;
  const assertNotPaused = async (where) => {
    if (!matterId || !pauseDeployed) return;
    const p = await matterPauseWithClient(supabase, matterId);
    if (p?.notDeployed) { pauseDeployed = false; return; }
    if (p?.error) {
      throw new AssistantRefusal(
        'ai_pause_unknown',
        'Whether AI is paused on this matter could not be checked, so nothing further was ' +
        'sent — a matter is treated as paused until that is known. Try again in a moment.',
      );
    }
    if (p?.paused) {
      const err = new AssistantRefusal('ai_paused', aiPausedMessage(p));
      err.abortedAt = where;
      throw err;
    }
  };
  ...
  try {
    ...
    if (matterId) {
      ...
      // Before the session row, before choosePen, before any driver exists:
      // a paused matter is refused with nothing contacted and nothing opened.
      await assertNotPaused('before_pen');
    ...
    //
    // Two further narrowings, both subtractive, neither able to add anything:
    //   * `toolNames`, when the caller supplied one (the meeting route);
    //   * LEAVES_THE_MATTER on a SEALED matter — relay_feedback and
    //     move_document are dropped structurally rather than by asking the
    //     model nicely in the system prompt.
    const narrow = toolNames
      ? new Set(toolNames instanceof Set ? [...toolNames] : toolNames)
      : null;
    const sealedRun = isSealedTier(tier);
    const mayOffer = (n) =>
      (!narrow || narrow.has(n)) && !(sealedRun && LEAVES_THE_MATTER.has(n));

    const baseTools = charter ? narrowToolNames(charter.allowed_tools, ALLOWED_TOOLS) : [...ALLOWED_TOOLS];
    const runTools = new Set(baseTools.filter(mayOffer));
    const tools = [
      ...toAnthropicTools(runTools),
      ...CLIENT_ACTION_TOOLS.filter((t) => mayOffer(t.name)),
      ...(charter ? [] : CONFIRM_ACTION_TOOLS.filter((t) => mayOffer(t.name))),
      ...(charter || !mayOffer(FEEDBACK_TOOL.name) ? [] : [FEEDBACK_TOOL]),
    ];
    // Every name actually on this run's menu. The loop refuses anything else,
    // so a model that invents a name — or remembers one from a previous,
    // wider turn — cannot reach a branch below.
    ...
    // ── The PEN's own preamble, last thing before the driver and FIRST thing
    // in the prompt. Everything above this line — the Orchestrator, a
    // charter's appendix, the SecureSpace paragraph — is the feature's prompt
    // and stays byte-identical after the separator; this only ever prepends.
    //
    // Driven off `pen.preamble`, so a first-party pen (Tier A, and a Tier-B
    // escalation) is untouched, and a future pen that needs different framing
    // declares its own instead of this function growing a branch.
    // Idempotent: applyPenPreamble will not stack a second copy.
    system = applyPenPreamble(system, pen.preamble);
    ...
    for (let i = 0; i < MAX_ITERATIONS; i++) {
      rounds++;
      // BETWEEN ROUNDS. A run that was legal when it started is re-asked
      // before every further trip to the provider, so a pause set while the
      // model is mid-answer ends the turn at the next boundary instead of
      // running to completion. The one thing this cannot interrupt is the
      // round already in flight — see the note above the driver call.
      if (i > 0) await assertNotPaused(`round_${i}`);
      ...
      // is untouched.
      //
      // ⚠ THE ONE THING A PAUSE CANNOT INTERRUPT is this call. Once the
      // request is on the wire the provider has the prompt, and no switch
      // here can unsend it: the driver streams to completion (or to its own
      // failure) and the pause is honoured at the next boundary. The window
      // is one model round — bounded by MAX_OUTPUT_TOKENS (8,192) and, in
      // production, by the function's own maxDuration in vercel.json. There
      // is no AbortController on the drivers; adding one would shorten the
      // window but could not close it, because the provider has already been
      // given the text.
      let turn;
      try {
      ...
      for (const tu of toolUses) {
        // BEFORE EVERY TOOL CALL — ahead of the browser actions, the
        // confirm-gated writes and the feedback relay, not merely ahead of
        // callTool. A pause stops the whole turn here, mid-round.
        await assertNotPaused(`tool_${tu.name}`);

        usedTools.push(tu.name);
        const args = tu.input && typeof tu.input === 'object' ? tu.input : {};

        // Not on this run's menu (narrowed by a charter, by `toolNames`, or
        // by the seal): refused before any branch can act on it.
        if (!offered.has(tu.name)) {
    ...
    // A run stopped by the pause leaves its own line in the matter's Record,
    // beside the completion row: which session, what stopped it, and at which
    // boundary. Metadata only, best-effort, and silent where 064 is not
    // deployed (lib/ledger.mjs treats that as "not deployed", never a
    // failure) — the Record is never the reason a refusal fails to land.
    // Only when a run actually started. A turn refused before the pen was
    // chosen was never a run, and calling it `run.aborted` would put a
    // sentence in the Record that is not true of what happened; the refusal
    // itself is recorded by recordAssistant below either way.
    if (code === 'ai_paused' && matterId && rounds > 0) {
      await record(supabase, {
        kind: 'run.aborted',
        matterId,
        sessionId: ledger.session?.id ?? null,
        actor: { kind: charter ? 'charter' : 'user', ref: charter ? charter.id : null, label: charter?.name ?? null },
        payload: { by: 'pause', round: rounds, at: err?.abortedAt ?? null, tools_used: usedTools.length },
      }).catch(() => {});
    }
    const recorded = await recordAssistant(supabase, ledger, {
      pen, tier, answer, usedTools, rounds, usage, charter, matterId,
  ...
  try {
    if (sessionId) {
      ...
      if (data && data.matterspace_id === matterId && data.status === 'open') {
        // Migration 072 made this row immutable: the tier that governed the
        // session, the matter, the owner and the snapshots are the AI record
        // and are not edited after they are written. `updated_at` still has
        // to move, so it moves through the one door that is allowed to move
        // it. Fire-and-forget exactly as before — and before 072 is pasted
        // the RPC is simply absent (PGRST202), which is swallowed here just
        // like the UPDATE's own failure was.
        supabase.rpc('ai_session_touch', { p_session: data.id, p_title: null })
          .then(() => {}, () => {});
...
async function recordAssistant(supabase, ledger, {
  ...
  if (matterId) {
    const led = await record(supabase, {
      ...
      payload: {
        ...
        // Which pen-owned instructions were in force for this turn. A version
        // string, never the prose: the Record stays metadata-only, and an AI
        // Use Record can resolve the string to the exact text in
        // lib/pen-preambles.mjs. Absent on a pen that declares none (Tier A,
        // and a recorded escalation), because there were none.
        ...(pen?.preamble ? { pen_preamble: pen.preamble.version } : {}),
  ...
  const content = {
    ...
    // The same version string as the Record row above, in the privileged
    // record itself (051's jsonb `content` — no new column, no migration), so
    // a single exchange can be shown with the instructions that governed it.
    ...(pen?.preamble ? { pen_preamble: pen.preamble.version } : {}),
    ...(error ? { error } : {}),
  };
