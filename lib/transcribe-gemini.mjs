...
async function generate({ apiKey, model, prompt, fileUri, mimeType, maxRetries = 4 }) {
  ...
  let attempt = 0;
  for (;;) {
    // A silently-dead connection never errors and never closes — the SSE read
    // just hangs forever. Two aborts turn hangs into retryable errors:
    //   * 120s without any bytes (dead connection), and
    //   * a 8-minute per-attempt wall clock (a connection that trickles
    //     keepalive bytes forever resets the idle timer but never finishes —
    //     observed on Fleming jail-call audio, 2026-07-22).
    const controller = new AbortController();
    const attemptStart = Date.now();
    let lastActivity = Date.now();
    const idleTimer = setInterval(() => {
      if (Date.now() - lastActivity > 120_000) controller.abort(new Error('gemini stream idle >120s'));
      if (Date.now() - attemptStart > 480_000) controller.abort(new Error('gemini attempt exceeded 8min wall clock'));
    }, 10_000);
    try {
...
// Does a transcript contain anything spoken (or, for video, seen)? Gemini
// marks non-speech in brackets — "[00:00] [silence]", "[background noise]",
// "[music]" — exactly as its prompt asks, so a silent or noise-only recording
// comes back as a line of markers and nothing else. Strip timestamps and
// short bracketed / parenthesised markers; whatever words remain are content.
// A video's "VISUAL:" notes count as content: no speech but visible events is
// still a transcript worth indexing. (2026-09-06: the nightly suite's silent
// clip was being indexed as a passage reading "[00:00] [silence]".)
export function hasSpokenContent(text) {
...
// or [] when the recording held nothing spoken (the caller stores it with a
// reason rather than indexing a marker). Matches the shape ingest-core
// expects from an extractor so the transcript flows straight into
// chunk -> embed -> insert.
export async function transcribeMedia(buf, {
  ...
  try {
    ...
    if (!hasSpokenContent(clean)) {
      onProgress({ message: `Transcript: no speech found (${clean.slice(0, 60) || 'empty'})` });
      return [];
    }
