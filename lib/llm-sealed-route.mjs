 ...
 *   { pen, send() }           — the sealed route. `send()` performs exactly
 ...
 *                               passthrough can pipe it unchanged.
 *
 ...
 */
export function sealedRouteFor({ gate, provider, model, body, env = process.env }) {
  ...
  return {
    ...
    send: () => sendSealed({ pen, creds, neutral, shape, env }),
  };
}
...
function sealedMaxTokens(neutral, route) {
  const headroom = route === 'chat' && neutral.forcedTool ? THINKING_HEADROOM_TOKENS : 0;
  return Math.min(neutral.maxTokens + headroom, SEALED_MAX_OUTPUT_TOKENS);
}
 ...
 * kinds of caller.
 */
function renderChatBody(neutral, pen) {
  const messages = [];
  ...
  const body = {
    ...
    max_tokens: sealedMaxTokens(neutral, 'chat'),
  };
 ...
 */
function renderMessagesBody(neutral, pen) {
  const body = {
    model: pen.model,
    max_tokens: sealedMaxTokens(neutral, 'messages'),
    stream: true,
...
async function sendSealed({ pen, creds, neutral, shape, env }) {
  const route = pen.route === 'messages' ? 'messages' : 'chat';
  const url =
    route === 'messages'
  ...
  const bodyText = JSON.stringify(route === 'messages' ? renderMessagesBody(neutral, pen) : renderChatBody(neutral, pen));
