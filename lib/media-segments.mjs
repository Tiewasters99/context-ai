...
export async function transcribeInSegments(buf, ext, {
  ...
  // A part with nothing spoken comes back empty (transcribeMedia strips the
  // "[silence]" markers). All parts empty = a recording with no speech: no
  // pages, so the caller stores it with a reason instead of indexing seam
  // lines. Some parts empty: say so in place, so the timeline stays honest.
  if (parts.every((p) => !p.text)) return { pages: [], segments: mp3s.length, durationSec };
  for (const p of parts) if (!p.text) p.text = '[00:00] [silence]';
  return {
