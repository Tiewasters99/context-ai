...
import {
  isSealedTier, matterTierWithClient, providerAllowed,
...
export async function meetingModelDecision(supabase, meetingId, { provider } = {}) {
  ...
  // The AI pause (migration 070), asked BEFORE the tier's allowlist and
  // answered with its own code.
  //
  // The order matters for the same reason it matters in gateLlmRequest: a
  // caller that treats a refusal as an invitation to try the sealed route
  // instead — which is what a sealed meeting arm does — must be able to tell
  // "this provider is outside the seal" from "this matter is switched off".
  // So a paused meeting refuses with code 'ai_paused' and `paused: true`, and
  // a sealed arm gates on that rather than on `ok` alone. Belt and braces:
  // any sealed arm reaches the pen through runAssistantStream, which asks the
  // pause again before a driver exists — so "zero requests" holds either way.
  let pause = null;
  try {
    pause = await matterPauseWithClient(supabase, meeting.matterspace_id);
  } catch {
    pause = { error: 'pause lookup threw' };
  }
  if (pause?.error) {
    return {
      ok: false, status: 503, code: 'ai_pause_unknown', tier, paused: true, matterId: meeting.matterspace_id,
      message:
        'Whether AI is paused on this matter could not be checked, so nothing was sent — a ' +
        'matter is treated as paused until that is known. The meeting itself keeps running ' +
        'and stays recorded. Try again in a moment.',
    };
  }
  if (pause?.paused) {
    return {
      ok: false, status: 403, code: 'ai_paused', tier, paused: true, matterId: meeting.matterspace_id,
      message: `${aiPausedMessage(pause)} The meeting itself keeps running and stays recorded.`,
    };
  }
