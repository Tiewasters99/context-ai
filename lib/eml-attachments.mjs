...
import { decodeHeaderValue } from './eml-extract.mjs';
...
// its header block only. It names a file rather than indexing anything, but a
// name is read by a person: it goes through the same header decoding as the
// message itself, so an em dash is an em dash and not "â€".
function nestedSubject(bytes) {
  ...
  if (!m) return '';
  const charset = /charset\s*=\s*"?([^";\s]+)"?/i.exec(block)?.[1];
  return decodeHeaderValue(m[1].trim(), charset).slice(0, 120);
}
