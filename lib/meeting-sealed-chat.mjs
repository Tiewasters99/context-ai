...
export async function runSealedMeetingTurn({
  ...
  // No question in the history means the framing turn would BE the last user
  // message — and the harness records the last user message in ai_messages, so
  // the transcript would land in the ledger as though the user had typed it.
  // The UI always sends the question; anything else is malformed, and saying so
  // is better than writing a transcript into a row that means something else.
  const asked = (Array.isArray(messages) ? messages : [])
    .some((m) => m && m.role === 'user' && typeof m.content === 'string' && m.content.trim());
  if (!asked) {
    return {
      streamed: false,
      notice: null,
      ...
      refusal: {
        ...
        body: {
          error: 'no_question',
          tier: 'B',
          message: 'There is no question here to answer — nothing was sent. Type what you want to '
            + 'ask about the meeting and the sealed pen will answer it.',
        },
      },
    };
  }
