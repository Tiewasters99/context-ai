 ...
 *
 * Migration 094 (the security build, docs/specs/SECURITY-BUILD-2026-09-26.md
 * §3) adds fifteen more in one go, so no later slice reopens the constraint.
 * S1 writes the first three, on the ACCOUNT chain:
 *   auth.factor_enrolled   {factor_id, factor_type}
 *   auth.factor_unenrolled {factor_id, factor_type}
 *   auth.stepup            {factor_type, matter_id}
 * The rest are defined for S2–S6 and written by nothing yet.
 *
 * Migration 100 (the Brief Desk, docs/specs/BRIEF-DESK-2026-09-26.md §3.1)
 * adds three, all matter-bound. Autosave writes none of them.
 *   draft.snapshot  {document_id, snapshot_id, sha256, label}
 *   cite.checked    {document_id, run_id, snapshot_id, counts}          (D3)
 *   draft.exported  {document_id, snapshot_id, destination: 'md'|'docx'|'agent', task_id?}
 */
...
/** The fifteen kinds migration 094 added, in the order it lists them. The
 *  end is explicit, so a later migration's kinds never count as 094's. */
export const KINDS_094 = Object.freeze(
  EVENT_KINDS.slice(EVENT_KINDS.indexOf('auth.factor_enrolled'),
    EVENT_KINDS.indexOf('tripwire.cleared') + 1),
);

/** The three kinds migration 100 (the Brief Desk) added. */
export const KINDS_100 = Object.freeze(
  EVENT_KINDS.slice(EVENT_KINDS.indexOf('draft.snapshot')),
);
...
export const ACCOUNT_EVENT_KINDS = Object.freeze([
  ...
  'ai.paused',
  'ai.resumed',
  // 094: facts about a person, not a matter. matter.disconnected, file.* and
  // share.expired stay matter-bound, and the database refuses them here too.
  'auth.factor_enrolled',
  'auth.factor_unenrolled',
  'auth.stepup',
  'auth.new_device',
...
const KIND_MIGRATIONS = Object.freeze({
  ...
  ...Object.fromEntries(KINDS_094.map((k) => [k, '094_security_kinds_and_stepup.sql'])),
  ...Object.fromEntries(KINDS_100.map((k) => [k, '100_brief_desk.sql'])),
});
...
export async function recordAccount(supabase, {
  ...
  if (error) {
    ...
    // 094 not pasted yet: either the CHECK constraint refuses the kind (23514)
    // or 072's account-legal list does (22023, its own message). Both mean
    // "the migration is not in this database", never a real write failure —
    // and only for a kind 094 itself adds.
    if (isKindNotAdmitted(error, eventKind) || isAccountKindNotAdmitted(error, eventKind)) {
      if (!warnedKinds.has(`account:${eventKind}`)) {
        warnedKinds.add(`account:${eventKind}`);
        console.warn(
          `[ledger] this database's account chain does not admit '${eventKind}' yet — ` +
          'those rows are not being recorded. Paste supabase/migrations/094_security_kinds_and_stepup.sql, ' +
          "then run: notify pgrst, 'reload schema';",
        );
      }
      return { ok: false, notDeployed: true, kindNotAdmitted: true, error };
...
/** 072's account-legal list refusing a kind 094 adds: the paste is pending. */
export function isAccountKindNotAdmitted(error, kind) {
  if (!error || !KINDS_094.includes(kind)) return false;
  if (String(error.code ?? '') !== '22023') return false;
  return /does not belong on an account chain/.test(String(error.message ?? ''));
}
