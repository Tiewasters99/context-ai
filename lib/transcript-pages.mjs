...
import { isStampLine } from './court-stamps.mjs';
...
/**
 * Where the reporter's line-number column starts and ends on one page.
 *
 * The column is the run of "12   Q.  Where were you" lines down the left
 * margin. What sits ABOVE it is the running header — and the page number the
 * reporter printed there is a 1-2 digit token alone on a line, shaped exactly
 * like a line number and indistinguishable from one until you know where the
 * column began. Both the zone reader below and `parseTranscriptPage` need that
 * boundary, so it is read once, here.
 *
 * @param {string} pageText
 * @returns {{raw: string[], numbered: {index:number,n:number}[], start:number, end:number}}
 *   `start` and `end` are indices into `raw`; `start` is -1 when there is no
 *   column (a Veritext text layer carries no numbers at all).
 */
export function lineColumnBounds(pageText) {
  // Split on either terminator. A transcript printed to a text file on Windows
  // ends its lines CRLF, and a stray carriage return on the end of a line
  // defeats the end anchor in every line regex - here and in
  // parseTranscriptPage - which would drop a whole TXT printout to prose.
  const raw = String(pageText || '').replace(/[· ]/g, ' ').split(/\r?\n/);
  const numbered = [];
  for (let i = 0; i < raw.length; i++) {
    ...
    numbered.push({ index: i, n });
  }
  // The top of the line-number column is the first "1" or "2" that is FOLLOWED
  // by its successor. A page whose printed number happens to be 1 or 2 puts a
  // bare "1" in the header — at column 0, because a PDF text layer carries no
  // indentation — and taking that as line 1 would swallow the header zone on
  // exactly the first pages of every transcript.
  let start = -1;
  let end = -1;
  for (let k = 0; k < numbered.length; k++) {
    if (start < 0 && numbered[k].n <= 2 && numbered[k + 1]?.n === numbered[k].n + 1) {
      start = numbered[k].index;
    }
    end = numbered[k].index;
  }
  return { raw, numbered, start, end };
}
...
export function pageZones(pageText) {
  const { raw, start: firstNum, end: lastNum } = lineColumnBounds(pageText);
 ...
 */
/**
 * Page-number candidates from a page's annotation overlay text: every line is
 * header/footer by construction, so there are no zones to find. The court's
 * stamp is excluded here as on the page.
 */
export function overlayCandidates(overlayText) {
  ...
  for (const raw of String(overlayText).split(/\r?\n/)) {
    const line = raw.trim();
    if (!line || isStampLine(line) || /\bpage\s+\d{1,4}\s+of\s+\d{1,4}\b/i.test(line)) continue;
...
export function pageNumberCandidates(pageText) {
  ...
  // A transcript filed as an exhibit carries the court's stamp on every sheet
  // — "Case 2:25-cv-02287 Document 66-7 Filed 03/23/26 Page 90 of 157" — and
  // its "Page 90" counts the PDF, not the reporter. Read as a candidate it
  // scored highest ("Page N" is the strongest shape), fitted every page at
  // offset 0 with "high" confidence, and cited the reporter's page 89 as 90
  // (DeCamara, ECF 66-7, found in the brief's cite-check 2026-09-24). On a
  // stamped page the stamp's own lines are not the reporter speaking.
  const ecfStamped = /\bcase\b[^\n]{0,80}\bdocument\b[^\n]{0,60}\bfiled\b/i.test(pageText);
  for (const line of [...header, ...footer]) {
    if (ecfStamped && (isStampLine(line) || /\bpage\s+\d{1,4}\s+of\s+\d{1,4}\b/i.test(line) && /\b(case|document|filed)\b/i.test(line))) continue;
    if (ecfStamped && /^page:?\s*\d{1,4}\s+of\s+\d{1,4}$/i.test(line)) continue;
    ...
    // joins the whole baseline into one string with NO separator, so it can
    // arrive as "DESMOND BLAKE 15" or as "DESMOND BLAKE15". Both readings are
    // offered — the digits at the end, and the last few characters read
    // through the OCR-confusion map — and the sequence fit decides.
    if (!alone) {
      const digits = /(\d{1,4})\s*$/.exec(line);
      if (digits) add(plainNumber(digits[1]), 1);
    }
    const tail = /([A-Za-z0-9|]{1,5})\s*$/.exec(line);
    if (tail) add(readNoisyNumber(tail[1]), 1);
  }
  return [...byValue.entries()].map(([value, score]) => ({ value, score }));
}
...
export function detectPrintedPages(pages, parsed = []) {
  ...
  for (const f of transcriptFacts) {
    f.candidates = pageNumberCandidates(pages[f.index].text);
    // A header/footer stamped as a Watermark annotation (lib/pdf-overlay-
    // text.mjs) is the reporter's number when the text layer has none.
    for (const c of overlayCandidates(pages[f.index].overlayText)) {
      const same = f.candidates.find((x) => x.value === c.value);
      if (same) same.score = Math.max(same.score, c.score);
      else f.candidates.push(c);
    }
  }
