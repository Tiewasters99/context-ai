...
// Westlaw cuts a long file name at about 100 characters, and a header can
// hand over a fragment. A name that shows either is not put in a cite.
const T6_TOKENS = new Set(Object.values(T6_WORDS).concat(Object.values(T6_RESTORE)).filter((w) => /[.']/.test(w)));
export function caseNameDoubt(name, source) {
  const s = String(name || '').trim();
  if (/\.\.\.|…/.test(s)) return 'truncated ("...")';
  if (/(?:\b(?:of|and|the|for|in|on|to)|[&,])\s*$/i.test(s)) return 'truncated (ends mid-name)';
  if (/ v\. *$/.test(s) || / v\. [^A-Za-z0-9]/.test(s)) return 'truncated (nothing after "v.")';
  const first = s.split(' ')[0];
  if (T6_TOKENS.has(first) && !/^(Cnty\.|Bd\.|Dep't|Comm'n|Ass'n|Nat'l|Int'l|Univ\.|Am\.|Gen\.|Fed\.|Pub\.|Consol\.|Cmty\.)$/.test(first)) return 'like a fragment (starts with an abbreviation)';
  if (source === 'Westlaw file name' && s.length > 95) return 'truncated (long file name)';
  // A word Westlaw clipped inside its own abbreviation ("Comn", "RCo").
  if (/\b[A-Z][a-z]{0,3}Co\b|\bComn\b|\bConst\b|\bex rel\b(?!\.)/.test(s)) return 'mangled (a clipped abbreviation)';
  return null;
}
...
export function bluebookCite(info, pages, levels) {
  ...
  const doubt = caseNameDoubt(info.case_name, info.case_name_source);
  if (doubt) return { cite: null, reason: `case name looks ${doubt}` };
  ...
  if (info.reporters.length) {
    ...
    if (Object.keys(spare).length && info.all_cites?.length) {
      // A U.S. cite must also fit the decision year: the synopsis cites other
      // U.S. cases, and "403 U.S. 388" is Bivens, not the case in hand.
      const extra = info.all_cites.filter((c) => !info.reporters.some((r) => r.volume === c.volume && r.reporter === c.reporter)
        && (c.reporter !== 'U.S.' || usVolumeFits(c.volume, info.date?.year)));
...
export function bluebookCitesFor(passages, westlaw, sourceFilename) {
  ...
  const cites = westlaw.pages.map((pg) => {
    ...
    const r = bluebookCite(info, pages, westlaw.levels);
    if (!r.cite && !reason) reason = r.reason;
    return r.cite;
  });
  return { info, cites, reason };
}
