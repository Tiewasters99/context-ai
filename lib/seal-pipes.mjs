...
import { isSealedTier, matterTierWithClient, sealedMatterIds } from './ai-tier-policy.mjs';
...
/**
 * Split a set of matter ids into the ones a query may be embedded for and the
 * ones it may not. Search uses this to keep working inside a sealed matter:
 * migration 056's stage B (full-text) needs no embedding and no network at
 * all, so a sealed matter still answers — it just answers on words rather
 * than on meaning, and says so.
 *
 * Asked from the sealed side, not the matter side. Walking each id's ancestor
 * chain would be a handful of round trips PER MATTER on an app-wide search —
 * dozens of queries before a single result, every time, to establish that
 * nothing is sealed. sealedMatterIds asks the one question that is usually
 * cheap: which roots are sealed (one query, normally zero rows), and only then
 * expands their descendants. Nothing sealed anywhere costs exactly one query.
 */
export async function partitionSealedMatters(supabase, matterIds) {
  const sealedIds = await sealedMatterIds(supabase);
  if (sealedIds.size === 0) return { open: [...matterIds], sealed: [] };
  const open = [];
  const sealed = [];
  for (const id of matterIds) (sealedIds.has(id) ? sealed : open).push(id);
  return { open, sealed };
}
