...
export function estimateUploadItem(item, { tier = 'A', env = {} } = {}) {
  ...
  if (OCRABLE_IMAGE_EXTENSIONS.includes(ext)) {
    ...
    const cents = route
      ? estimateOcrCents(1, { route }) + embedCentsForChars(ASSUMED_CHARS_PER_PAGE)
      : 0;
    basis.push(route
      ? 'one page, read by OCR'
      : 'this matter sends no page to an OCR provider, so the picture is stored unread');
    return {
      name, ext, kind: 'image', bytes,
      pages: 1, pagesKnown: true, minutes: 0, minutesKnown: true,
      lowCents: cents, highCents: cents,
      basis,
    };
  }
...
/**
 * Which declaration a POST should carry — and the distinction that makes
 * requirement 5 true rather than nearly true.
 *
 *   null       the file WAS measured and no quote was owed. Send nothing. The
 *              request is the one this endpoint has always taken.
 *   an object  a quote was shown and confirmed. Send it.
 *   undefined  the file was never measured, because there was no drop and no
 *              dialog — an app-generated document (a Record export, a combined
 *              exhibit PDF, a trial outline), or a re-run from a row. Form one
 *              from the only two facts available, and only if it is big enough
 *              for the handler to ask about.
 *
 * The three must not collapse into one. A 13 MB PDF of fifty photographed
 * pages measures at fifty and is quiet; its size ALONE reads as 222 pages
 * (ASSUMED_BYTES_PER_PAGE errs high on purpose). Treating "measured, small"
 * as "unknown" would send a page count the browser had already measured as
 * wrong, on an upload that is supposed to be untouched.
 */
export function chooseDeclaration(supplied, { name, bytes } = {}) {
  if (supplied === null) return null;
  if (supplied) return supplied;
  const item = estimateUploadItem({ name, bytes });
