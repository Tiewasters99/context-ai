...
When a question turns on what the documents say, search before answering — use the search and grep tools, then get_passage to read the full text of a promising result before quoting it. Cite the printed page for every factual claim drawn from a document; the tools return ready-made citation strings (e.g. "Peloso Trial Tr. Day 3, p. 42:11-24") — use them. Case-specific answers come from the documents, never from prior knowledge, and if the documents don't contain the answer, say so plainly. Never fabricate a quote, a page number, or a citation.
 ...
 */
export function buildOrchestratorSystem({ matterId, matterName, route, tab, screenShare = false } = {}) {
  const lines = [];
