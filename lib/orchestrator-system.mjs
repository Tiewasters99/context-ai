 ...
 * @param {number} [ctx.page]         the page with the most of the screen, 1-based
 * @param {number} [ctx.pageCount]
 * @param {Array<{page:number, share?:number, text?:string}>} [ctx.pages] the pages on screen, most visible first
 * @param {boolean} [ctx.indexed]     false when the copy has no indexed passages
 * @param {string} [ctx.sourceDocumentId] the indexed original an unindexed copy was made from
 */
export function buildOrchestratorSystem({
  matterId, matterName, route, tab, today, screenShare = false,
  documentId, documentTitle, page, pageCount, pages, indexed, sourceDocumentId, unindexedReason,
} = {}) {
  const lines = [];
  ...
  if (docId) {
    ...
    const count = Number.isInteger(pageCount) ? ` (${pageCount} pages)` : '';
    lines.push(`- They have ${title} open in the reader (document id ${docId})${count}. "This book", "this page", "this passage" mean this document.`);
    const shown = Array.isArray(pages) ? pages.filter((p) => p && Number.isInteger(p.page)) : [];
    if (shown.length) {
      const names = shown.map((p) => `page ${p.page}`);
      const most = shown[0].page;
      const where = shown.length === 1
        ? `The reader's scroll puts ${names[0]} on their screen`
        : `The reader's scroll puts ${names.join(' and ')} on their screen, ${most} mostly`;
      lines.push(`- ${where} — an estimate from geometry; if they say otherwise, believe them.`);
      for (const p of shown) {
        if (str(p.text)) lines.push(`- Page ${p.page} reads:\n"""\n${str(p.text)}\n"""`);
      }
    } else if (Number.isInteger(page)) {
      lines.push(`- The reader's scroll puts page ${page} on their screen — an estimate; if they say otherwise, believe them.`);
    }
    const sourceId = str(sourceDocumentId);
    if (indexed === false && unindexedReason === 'generated') {
      lines.push(`- This copy is a generated deliverable (a PDF edited in the vault) and is NOT indexed: search, grep and get_passage find nothing in it, so do not conclude the text is missing from the record. Only the pages on their screen are available to you from this copy.` +
        (sourceId
          ? ` It was made from document ${sourceId}, which is indexed — search that with document_ids: ["${sourceId}"], then get_passage, for the rest of the text. Its page numbers are the original's and may differ from this copy's where pages were cut or put in.`
          : ' No original is recorded for it.'));
    } else if (indexed === false) {
      lines.push(`- This document has NO indexed passages — its import never ran, is still running, or produced nothing — so search, grep and get_passage find nothing in it. Only the pages on their screen are available to you. Say so plainly if it matters; check_ingest_status gives the real picture, and ingest_document can queue the import if they want it searchable.`);
    } else {
      lines.push(`- To read beyond what is on their screen, search with document_ids: ["${docId}"], then get_passage.`);
    }
  }
