...
export function signSupabaseUserJwt(user_id) {
  ...
  const payload = {
    ...
    // Migration 094: a sealed matter is closed to a browser session that has
    // not confirmed a second factor. Tokens minted HERE are for connected
    // assistants and the extension, which the seal already governs (it refuses
    // them with a sentence and records the attempt), so the gate exempts a
    // token carrying this stamp — and only one carrying it. Supabase Auth
    // never issues it; forging it needs the key that signs this token.
    cs_via: 'connector',
  };
