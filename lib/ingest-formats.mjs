...
export const SUPPORTED_EXTENSIONS = [
  ...
  // Rich Text — what Westlaw and Lexis hand out, and older Word exports.
  // Read through lib/rtf-text.mjs (2026-09-07); before that it fell to the
  // plain-text path and indexed its markup.
  '.rtf',
  // Legacy Word (Word 97–2003, and what Westlaw names "Word"). The bytes
  // decide how it is read — a real Word binary through word-extractor, or
  // RTF / .docx / PDF / HTML saved under a .doc name through their own
  // readers (lib/doc-extract.mjs). Refused until 2026-09-23.
  '.doc',
...
export const TEXT_STATUS = Object.freeze({
  ...
  ARCHIVE: 'archive',
  // A deliverable a tool built and filed store-and-display (a chart's SVG)
  // — its sources are the searchable copies. Recorded so the row says why
  // it holds no passages (before 2026-09-07 it said nothing and the monitor
  // escalated it).
  GENERATED: 'generated',
});
...
const TEXT_STATUS_TEXT = {
  ...
  media_no_transcript: {
    ...
    detail: 'The recording is kept as-is: it held no speech to transcribe, or transcription was not configured where it ran.',
  },
  binary_stored: {
  ...
  generated: {
    ...
    detail: 'Built by a tool and kept to open or download; the documents it was made from are the searchable copies.',
  },
};
...
// -----------------------------------------------------------------------------
// Vectors owed — documents.metadata.embedding_pending (2026-09-06)
// -----------------------------------------------------------------------------
//
// A sealed matter's embedding route can be fully configured and still have
// nothing listening: the SageMaker endpoint behind it is switched off between
// sealed sessions to stop its hourly meter, or is still coming up. The
// document finishes anyway — text extracted and chunked locally, indexed for
// full-text search — and the row records that its vectors are owed:
//
//   metadata.embedding_pending = {
//     route:    'voyage-4-sagemaker',   // the route that was configured
//     model:    'voyage-4',             // the space the vectors will join
//     passages: 12,                     // how many still have no vector
//     reason:   '…is not in service…',  // the endpoint's answer, in words
//     at:       ISO,                    // when it was recorded
//   }
//
// scripts/reembed-matter.mjs fills the vectors and clears the record. A later
// successful ingest run clears it too. Search works on words meanwhile.

// Plain-language label + detail for an embedding_pending record.
export function describeEmbeddingPending(pending) {
  if (!pending || typeof pending !== 'object') return null;
  const n = Number(pending.passages) || 0;
  const reason = String(pending.reason || 'The sealed embedding endpoint was not in service.').split('\n')[0].trim();
  return {
    label: `Searchable by words — ${n ? `${n} passage${n === 1 ? '' : 's'} ` : ''}awaiting sealed embeddings`,
    detail: `${reason} Full-text search works now; meaning-based search catches up once the endpoint is back and ` +
      'scripts/reembed-matter.mjs has run for this matter. Nothing was sent to any other provider.',
  };
}
...
const SUPPORTED_TYPES_BASE =
  'PDF, Word (.docx, .doc, .rtf), text (.txt, .md, .csv, .html), spreadsheets (.xlsx), ' +
...
// "~$Brief.docx": the lock file Word (and Excel, PowerPoint) writes beside a
// document while it is open. It holds the editor's name, never content, and
// it rides along whenever a whole folder is dragged into the Vault.
...
// -----------------------------------------------------------------------------
// Refused, but with the thing to do instead (2026-09-20)
// -----------------------------------------------------------------------------
//
// A refusal that only says "can't read it" costs the person a support email.
// These are the formats a lawyer actually has on day one — an iPhone photo, a
// pre-2007 Word file, an Outlook message — and each one names the alternative
// in the words of the application that made it. A file type is listed here
// ONLY because the pipeline genuinely cannot read it; the rule is refuse at
// selection with a named alternative, never accept and index to nothing.
//
// .heic / .heif — measured 2026-09-20, not assumed. sharp 0.34 (libvips 8.17,
//   libheif 1.20.2) reads an iPhone .heic's *container* — metadata says
//   3024×4032, compression "hevc" — and then fails to decode a single pixel
//   ("source: bad seek to …"): the prebuilt binary carries the AV1 decoder
//   (AVIF) and no HEVC decoder, and sharp itself advertises the fact
//   (sharp.format.heif.input.fileSuffix === ['.avif']). Accepting .heic would
//   therefore file every iPhone photo as an undecodable image with zero
//   passages — the silent half-acceptance this module exists to prevent. The
//   iPhone setting named below makes the camera write JPEG, which the OCR
//   path already reads (a photographed page is a gate in the nightly suite).
// .xls / .ppt — the pre-2007 binary Excel and PowerPoint formats (OLE
//   compound files). Nothing here parses them: SheetJS is wired for .xlsx.
//   Excel/PowerPoint still open them and Save As the modern format. (.doc,
//   the Word one, IS read since 2026-09-23 — lib/doc-extract.mjs.)
// .msg — Outlook's own message format, also an OLE compound file, and not
//   MIME: mailparser (which reads the .eml the Vault accepts) cannot open it.
export const UNSUPPORTED_HINTS = Object.freeze({
  '.heic': ' iPhone photos saved in High Efficiency format cannot be opened here.' +
    ' On the phone: Settings › Camera › Formats › Most Compatible makes new photos JPEG.' +
    ' For a photo you already have, email or message it to yourself — iOS sends a JPEG copy — and upload that.' +
    ' (AirDrop and the Files app keep the original HEIC, so those do not help.)',
  '.heif': ' High Efficiency images cannot be opened here. Export or share the picture as JPEG or PNG and upload that.',
  '.dot': ' Save the legacy Word template as .docx and upload that.',
  '.xls': ' Save the legacy Excel workbook as .xlsx and upload that.',
  '.ppt': ' Save the legacy PowerPoint file as .pptx and upload that.',
  '.msg': ' Outlook\'s own message format cannot be opened here. Open the message and print it to PDF' +
    ' (File › Print › Microsoft Print to PDF), or save it as .eml — dragging a message out of the new' +
    ' Outlook or Outlook for Mac gives .eml, while classic Outlook for Windows gives another .msg.' +
    ' Upload that; attachments can be uploaded on their own.',
  '.pst': ' An Outlook data file holds a whole mailbox, not a document. Export the messages you need' +
    ' as .eml or PDF and upload those.',
  '.ost': ' An Outlook data file holds a whole mailbox, not a document. Export the messages you need' +
    ' as .eml or PDF and upload those.',
  '.pages': ' Export it as PDF or .docx and upload that.',
  '.numbers': ' Export it as PDF or .xlsx and upload that.',
  '.key': ' Export it as PDF or .pptx and upload that.',
  '.wpd': ' Open the WordPerfect file and save it as .docx or PDF, then upload that.',
});

// The alternative to name in a refusal, or '' when there isn't a known one.
// Always begins with a space: callers append it to a sentence.
export function unsupportedHint(ext) {
  return UNSUPPORTED_HINTS[String(ext || '').toLowerCase()] || '';
}
...
// 'unsupported', 'lock_file' or 'empty'. A file with no extension at all is
// allowed through: the pipeline sniffs its magic bytes (email attachments
...
export function checkUpload({ name, size }, { zip = true } = {}) {
  ...
  if (isOfficeLockFile(name)) {
    return {
      code: 'lock_file',
      message: `"${name}" is a temporary Office lock file, not a document — Word writes it beside an open file and it holds no content. Upload the real document instead.`,
    };
  }
  // Five chat exports arrived as 0-byte files and sat as failures for weeks
  // (2026-09-07). Nothing can be made of no bytes; say so before they move.
  if (typeof size === 'number' && size === 0) {
    return {
      code: 'empty',
      message: `"${name}" is empty (0 bytes), so there is nothing to file. Save or export it again and upload the new file.`,
    };
  }
  if (bytes > cap) {
    return {
  ...
  if (ext && !ACCEPTED_EXTENSIONS.includes(ext)) {
    const hint = unsupportedHint(ext);
    return {
      code: 'unsupported',
