...
export function describeOcrPending(pending) {
  ...
  if (pending.held) {
    return {
      ...
      detail: `${reason} The typed pages are searchable; the scanned pages stay unread until a sealed OCR route exists.`,
    };
  }
...
//
// `zip`: whether this entry point expands archives. The web Vault does (it
// unpacks a .zip in the browser and files each entry); the MCP file_document
// tool does not, so for it a .zip is refused with the reason rather than
// stored as an unreadable blob.
export function checkUpload({ name, size }, { zip = true } = {}) {
  ...
  if (ext === '.zip' && !zip) {
    return {
      code: 'unsupported',
      message: `"${name}" is a .zip archive, which this path does not unpack. ` +
        'File its contents one at a time, or upload the archive through the web Vault, which expands it. ' +
        `Supported here: ${SUPPORTED_TYPES_BASE}.`,
    };
  }
  if (ext && !ACCEPTED_EXTENSIONS.includes(ext)) {
    ...
    return {
      ...
      message: `"${name}" is a ${ext} file, which the Vault can't read.${hint} ` +
        `Supported: ${zip ? SUPPORTED_TYPES_SUMMARY : SUPPORTED_TYPES_BASE}.`,
    };
  }
  return null;
}
