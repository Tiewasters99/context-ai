...
export default function UploadEstimateDialog({
  ...
  return (
    // Escape cancels (nothing has been uploaded, so leaving must be
    // effortless); the backdrop does not — it never did.
    <CardDialog
      storageKey="cs.dialog.uploadEstimate"
      z={50}
      maxWidth={512}
      onClose={onCancel}
      title={<>Upload {estimate.files.toLocaleString()} file{estimate.files === 1 ? '' : 's'}?</>}
      label="Upload files?"
      subtitle={<>Shown because {reasons.join(', and ')}. Smaller uploads go straight through.</>}
    >
