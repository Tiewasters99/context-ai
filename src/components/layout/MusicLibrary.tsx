...
export default function MusicLibrary({
  ...
  return (
    <CardDialog
      storageKey="cs.dialog.musicLibrary"
      z={60}
      maxWidth={820}
      onClose={onClose}
      // A YouTube link half-pasted is not lost to a stray click outside.
      closeOnBackdrop={!(showAddYt && (ytUrl.trim() || ytName.trim()))}
      icon={<MusicIcon size={18} className="text-[#e8b84a]" strokeWidth={1.75} />}
      title="Music Library"
      subtitle={
        <>
      ...
      actions={
        <>
          ...
          >
            <Video size={13} /> <span className="hidden sm:inline">Add YouTube</span>
          </button>
          <button
          ...
          >
            <Upload size={13} /> <span className="hidden sm:inline">{uploading ? 'Uploading…' : 'Upload your own'}</span>
          </button>
        </>
      }
