...
export default function BucketizerEvidenceRunDialog({
  ...
  return (
    // Escape cancels; the backdrop does not — it never did, and a stray click
    // is not a decision about a bill.
    <CardDialog
      storageKey="cs.dialog.bucketizerEvidenceRun"
      z={50}
      maxWidth={512}
      onClose={onCancel}
      icon={<Quote className="w-4 h-4 text-[#d4a054]" />}
      title={<>Find evidence in {estimate.pairs.toLocaleString()} document–issue pairing{estimate.pairs === 1 ? '' : 's'}?</>}
      label="Find evidence?"
    >
