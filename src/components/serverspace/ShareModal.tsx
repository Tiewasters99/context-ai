...
export default function ShareModal({ scope, scopeId, scopeName, onClose }: ShareModalProps) {
  ...
  return (
    <CardDialog
      storageKey="cs.dialog.share"
      z={70}
      maxWidth={540}
      onClose={onClose}
      // An address typed is not lost to a stray click outside.
      closeOnBackdrop={!email.trim()}
      icon={<UserPlus size={16} className="text-[#e8b84a]" strokeWidth={1.75} />}
      title={<span className="block truncate">Share {cfg.label} "{scopeName}"</span>}
      label={`Share ${cfg.label} "${scopeName}"`}
      subtitle={cfg.help}
      bodyClassName="p-0 flex flex-col"
      footerClassName="px-6 py-3 text-[10px] text-white/40 text-center"
      footer={
        <>
